<template>
  <div class="tickers-analist-tab">
    ...
    <div class="current-prices">
      ...
      <div v-if="shouldShowBTC" class="price-box btc-box">
        ...
        <div class="conversions">
          ...
          <span>{{ (latestPrices.btcPriceUSD / latestPrices.pairPriceUSD)?.toLocaleString() || '0' }} {{ coin }}</span>
        </div>
      </div>

      <div v-if="shouldShowETH" class="price-box eth-box">
        ...
        <div class="conversions">
          ...
          <span>{{ (latestPrices.ethPriceUSD / latestPrices.pairPriceUSD)?.toLocaleString() || '0' }} {{ coin }}</span>
        </div>
      </div>

      <div class="stats-box price-box">
    ...
    <div v-if="shouldShowBTC || shouldShowETH" class="correlations-section">
      <h2 class="section-title">📈 Price Correlations</h2>
      <div class="correlation-grid">
        <div v-if="shouldShowBTC" class="correlation-card">
          <span class="corr-label">{{ coin }} ↔ BTC</span>
        ...
        <div v-if="shouldShowETH" class="correlation-card">
          <span class="corr-label">{{ coin }} ↔ ETH</span>
        ...
        <div v-if="shouldShowBTC && shouldShowETH" class="correlation-card">
    ...
    <div class="differences-section">
      ...
      <div class="diff-card lcx-diff">
        <h3 class="diff-title">{{ coin }} Price Changes</h3>
        <div class="diff-grid">
          <div class="diff-item">
            <span class="diff-label">10s</span>
            <span class="diff-value" :class="getDiffClass(deltas.pair10s)">{{ formatDiff(deltas.pair10s) }}</span>
          </div>
          <div class="diff-item">
      ...
      <!-- BTC Differences -->
      <div v-if="shouldShowBTC" class="diff-card btc-diff">
        <h3 class="diff-title">BTC Price Changes</h3>
        <div class="diff-grid">
          <div class="diff-item">
            <span class="diff-label">10s</span>
            <span class="diff-value" :class="getDiffClass(deltas.btc10s)">{{ formatDiff(deltas.btc10s) }}</span>
          </div>
          <div class="diff-item">
            <span class="diff-label">1m</span>
      ...
      <!-- ETH Differences -->
      <div v-if="shouldShowETH" class="diff-card eth-diff">
        <h3 class="diff-title">ETH Price Changes</h3>
        <div class="diff-grid">
          <div class="diff-item">
            <span class="diff-label">10s</span>
            <span class="diff-value" :class="getDiffClass(deltas.eth10s)">{{ formatDiff(deltas.eth10s) }}</span>
          </div>
          <div class="diff-item">
            <span class="diff-label">1m</span>
...
// Determine which coins to show based on selected coin
const shouldShowBTC = computed(() => props.coin !== 'BTC');
const shouldShowETH = computed(() => props.coin !== 'ETH');
const shouldShowPair = computed(() => true); // Always show selected pair

// Calculate price deltas (updated for 10s intervals)
const deltas = computed(() => {
  if (tickerHistory.value.length < 2) {
    return {
      pair10s: 0, pair1m: 0, pair1h: 0, pair1d: 0,
      btc10s: 0, btc1m: 0, btc1h: 0, btc1d: 0,
      eth10s: 0, eth1m: 0, eth1h: 0, eth1d: 0
    };
  }
  ...
  return {
    ...
    pair10s: ((latest.pairPriceUSD - price10s.pairPriceUSD) / price10s.pairPriceUSD) * 100,
    ...
    // BTC deltas
    btc10s: ((latest.btcPriceUSD - price10s.btcPriceUSD) / price10s.btcPriceUSD) * 100,
    ...
    // ETH deltas
    eth10s: ((latest.ethPriceUSD - price10s.ethPriceUSD) / price10s.ethPriceUSD) * 100,
...
function updateChart() {
  ...
  // Build datasets dynamically based on which coins should be shown
  const datasets = [];

  // Always show selected pair
  datasets.push({
    label: `${props.coin} (USD)`,
    data: pairData,
    borderColor: '#00ffff',
  ...
  chartInstance = new Chart(chartCanvas.value, {
    ...
    options: {
      ...
      scales: {
        ...
        'y-eth': {
          type: 'linear',
          display: false, // Keep hidden as third axis
          position: 'right',
          grid: {
            drawOnChartArea: false
          }
...
watch(() => [props.symbol, props.exchange], () => {
  fetchTickerHistory();
}, { deep: true });

onMounted(() => {
  fetchTickerHistory();
  // Refresh data every 10 seconds (matching tickerSaver interval)
  fetchInterval = setInterval(fetchTickerHistory, 10000);
});
...
<style scoped>
/* Same styles as tickersAnalist.vue but without page-specific styles */
.tickers-analist-tab {
  padding: 0;
}
