...
async function analyzeMarket() {
  ...
  try {
    ...
    console.log('📊 Sending to ChatGPT (OpenAI):', {
...
const keyMetrics = computed(() => {
  ...
  return {
    ...
    expectedProfit: aiSuggestions.value.expectedProfitPerGrid || null
  };
});
</script>

<template>
  <n-card style="background: linear-gradient(135deg, #1a1a2e 0%, #0f3460 100%); border: 2px solid #10a37f;">
    <template #header>
      <div style="display: flex; align-items: center; gap: 10px;">
        <n-icon size="24" color="#10a37f">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-13h2v6h-2zm0 8h2v2h-2z"/>
          </svg>
        </n-icon>
        <span style="color: #10a37f; font-weight: 700; font-size: 14px;">💬 CHATGPT AI ADVISOR (GPT-4o)</span>
      </div>
    </template>
    ...
    <n-space vertical :size="12">
      <n-space>
        <n-button
          ...
          @click="analyzeMarket"
          :loading="loading"
          style="background: linear-gradient(135deg, #10a37f 0%, #0d8a6a 100%); border: none;"
        >
          <template #icon>
            <n-icon>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
          ...
          {{ loading ? 'Analyzing...' : 'Get ChatGPT Analysis' }}
        </n-button>
      ...
      <div v-if="loading" style="text-align: center; padding: 40px 0;">
        <n-spin size="large" />
        <p style="color: #a0aec0; margin-top: 16px;">ChatGPT is analyzing market conditions...</p>
      </div>
      ...
      <div v-else-if="aiAnalysis && aiSuggestions">
        ...
        <n-grid v-if="keyMetrics" :cols="3" :x-gap="8" :y-gap="8">
          <n-gi>
            <div style="background: rgba(16, 163, 127, 0.1); padding: 12px; border-radius: 8px; border: 1px solid rgba(16, 163, 127, 0.3);">
              <div style="color: #10a37f; font-size: 10px; font-weight: 600;">LOWER PRICE</div>
          ...
          <n-gi>
            <div style="background: rgba(13, 138, 106, 0.1); padding: 12px; border-radius: 8px; border: 1px solid rgba(13, 138, 106, 0.3);">
              <div style="color: #0d8a6a; font-size: 10px; font-weight: 600;">UPPER PRICE</div>
              <div style="color: #fff; font-size: 16px; font-weight: 700; margin-top: 4px;">
          ...
          <n-gi>
            <div style="background: rgba(72, 187, 120, 0.1); padding: 12px; border-radius: 8px; border: 1px solid rgba(72, 187, 120, 0.3);">
              <div style="color: #48bb78; font-size: 10px; font-weight: 600;">GRIDS</div>
              <div style="color: #fff; font-size: 16px; font-weight: 700; margin-top: 4px;">
        ...
        >
          ...
          Apply ChatGPT Recommendation to Form
        </n-button>

        <!-- Full Analysis Display -->
        <n-collapse arrow-placement="right">
          <n-collapse-item title="📊 Full ChatGPT Analysis" name="1">
      ...
      <div v-else style="text-align: center; padding: 40px 20px; color: #a0aec0;">
        <n-icon size="64" color="#10a37f">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z"/>
          </svg>
        </n-icon>
        <h3 style="color: #10a37f; margin-top: 16px;">ChatGPT-Powered Grid Bot Advisor</h3>
        <p style="font-size: 13px; margin-top: 8px;">
          Click "Get ChatGPT Analysis" to receive personalized grid bot configuration recommendations based on:
        </p>
