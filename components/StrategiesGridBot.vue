...
async function saveStrategy() {
  ...
  try {
    ...
    if (props.formData.name && props.formData.name.value !== undefined) {
...
async function handleDeleteStrategy() {
  ...
  try {
    await deleteStrategy(strategyPicker.value, props.userID);
...
async function handleDeleteAllStrategies() {
  ...
  try {
    await deleteAllStrategies(props.userID);
...
onMounted(async () => {
  if (!props.userID) {
    console.warn('⚠️ StrategiesGridBot: userID is not provided');
    return;
  }
  loadingStrategies.value = true;
  await loadStrategies(props.userID);
...
<template>
  <div class="strategies-section">
    ...
    <div v-if="strategiesList.length > 0" class="strategies-list">
      <div class="strategies-select-row">
        <n-select
          v-model:value="strategyPicker"
          :options="strategyPickerOptions"
          @update:value="selectStrategy(formData, strategyPicker)"
