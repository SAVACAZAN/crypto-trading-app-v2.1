...
export default defineEventHandler(async (event) => {
  try {
    ...
    const replicateApiKey = user.aiApiKeys?.replicate;
    if (!replicateApiKey) return { success: false, error: 'Replicate API key not configured. Please add your API key in the Profile page.' };
    ...
    const text = data.output?.join('') || JSON.stringify(data);
