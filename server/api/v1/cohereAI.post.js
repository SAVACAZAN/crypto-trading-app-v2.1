...
export default defineEventHandler(async (event) => {
  try {
    ...
    const cohereApiKey = user.aiApiKeys?.cohere;

    console.log('🔑 Cohere Request Debug:');
    console.log('   UserID:', userID);
    console.log('   Has API Key:', !!cohereApiKey);

    if (!cohereApiKey) {
      return { success: false, error: 'Cohere API key not configured. Please add your API key in the Profile page.' };
    }
    ...
    const cohereResponse = await callCohereAPI(prompt, cohereApiKey);
    const suggestions = parseResponse(cohereResponse);

    return {
      success: true,
      data: { analysis: cohereResponse, suggestions, marketContext }
    };
...
async function callCohereAPI(prompt, apiKey) {
  console.log('🤖 Calling Cohere API (FREE!)...');
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('❌ Cohere API Error Response:', error);
      throw new Error(`Cohere API error: ${response.status} - ${error}`);
    }

    const data = await response.json();
    const text = data.generations[0].text;
    console.log('✅ Cohere API Success! Response length:', text.length);
    return text;
