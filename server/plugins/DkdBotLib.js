...
import { DkdBotSchema } from "../models/DkdBot.schema";
    ...
    nitroApp.DkdBotLib = {
        async createBot(data) {
            ...
            if (data.ordersSide === 'buyOnly') {
                orders.push(await this.placeOrder(data.userID, data.exchange, data.symbol, data.PriceStart, 'buy', data.amountType, data.amount));
              
            } else if (data.ordersSide === 'sellOnly') {
                orders.push(await this.placeOrder(data.userID, data.exchange, data.symbol, data.PriceStart, 'sell', data.amountType, data.amount));
                
            }

            data['activeOrders'] = orders;

            await new DkdBotSchema(data).save();
        },
        ...
        async placeOrder(userID, exchange, symbol, price, side, amountType, amount) {
            let quantityPerGrid = await this.getQuantityPerGrid(price, amountType, amount, 1, 0);
            let orderResponse = await nitroApp.ccxtw.createOrder(userID, exchange, symbol, 'limit', side, quantityPerGrid, price);
        ...
        async fetchOrderStatus(userID, exchange, symbol, orderId) {
            let log = null;
            let data = null;
            let success = null;
            ...
            if (this.users.get(userID).get(exchange).has['fetchOrderStatus']) {
                try {
                    data = await this.users.get(userID).get(exchange).fetchOrderStatus(orderId, symbol);
                    success = true;
                } catch (e) {
                    data = null;
                    success = false;
                    if (e instanceof ccxt.NetworkError) {
                        ...
                        log = `Failed due to an exchange error: ${e.message}`;
                    } else {
                        log = `Failed with: ${e.message}`;
                    }
                }
            } else {
                log = `Exchange ${exchange} does not support fetchOrderStatus`;
                success = false;
            }
        ...
        async fetchdkdBots(userID, exchange, symbol) {
            let bots = await DkdBotSchema.find({ userID: userID,exchange: userID });
            return bots;
        }
    };
});
