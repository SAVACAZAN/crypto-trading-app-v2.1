...
const Exchange = ccxt.Exchange || ccxt.default?.Exchange;
const { ExchangeError, ExchangeNotAvailable, BadResponse, BadRequest, InvalidOrder, InsufficientFunds, AuthenticationError, RateLimitExceeded, InvalidAddress, DDoSProtection, BadSymbol } = ccxt;
...
export default class lcx extends Exchange {
    ...
    async fetchMarkets (params = {}) {
        ...
        for (let i = 0; i < markets.length; i++) {
            ...
            // Fix LCX precision interpretation - LCX returns decimal places, not step size
            const pricePrecisionFromAPI = this.safeFloat (market, 'pricePrecision');
            const pricePrecision = pricePrecisionFromAPI ? Math.pow(10, -pricePrecisionFromAPI) : undefined;

            const precision = {
                'amount': amountPrecision ? Math.pow(10, -amountPrecision) : undefined,
                'price': pricePrecision,
                'cost': costPrecision ? Math.pow(10, -costPrecision) : undefined,
            };
    ...
    async fetchBalance (params = {}) {
        ...
        // Build CCXT-compatible balance structure manually (parseBalance not working correctly)
        const result = {
            'info': response,  // Keep full API response
            'timestamp': Date.now(),
            'datetime': this.iso8601(Date.now()),
            'free': {},
            'used': {},
            'total': {},
        };
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const free = this.safeFloat (balance.balance, 'freeBalance');
            const used = this.safeFloat (balance.balance, 'occupiedBalance');
            const total = this.safeFloat (balance.balance, 'totalBalance');
            ...
            // Also add nested structure for each currency
            result[code] = {
                'free': free,
                'used': used,
                'total': total,
    ...
    sign (path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (api === 'private') {
            ...
            // FIXED: LCX API requires JSON.stringify for ALL requests, including GET with empty payload
            // Quartz format: method + '/' + path + JSON.stringify(payload)
            // For GET /api/balances: "GET/api/balances{}" NOT "GET/api/balances"
            // IMPORTANT: Must use explicit JSON.stringify because this.json({}) returns "" not "{}"
            const jsonQuery = Object.keys(query).length > 0 ? this.json(query) : '{}';
            let payload = method + '/' + path + jsonQuery;
            try {
                ...
                throw new Error('Failed to sign LCX request: ' + cryptoError.message);
            }
        }
