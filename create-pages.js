...
const networks = [
  ...
  { name: 'Theta', symbol: 'THETA', icon: '📺', api: 'generateThetaWallet', explorer: 'https://explorer.thetatoken.org/account/' },
  { name: 'Casper', symbol: 'CSPR', icon: '👻', api: 'generateCasperWallet', explorer: 'https://cspr.live/account/' },
  ...
  { name: 'NervosCKB', symbol: 'CKB', icon: '🧬', api: 'generateNervosCKBWallet', explorer: 'https://explorer.nervos.org/address/' },
  { name: 'AlephZero', symbol: 'AZERO', icon: '0️⃣', api: 'generateAlephZeroWallet', explorer: 'https://azero.dev/#/accounts/' },
  { name: 'Radix', symbol: 'XRD', icon: '🔴', api: 'generateRadixWallet', explorer: 'https://dashboard.radixdlt.com/account/' },
  { name: 'Nibiru', symbol: 'NIBI', icon: '🌌', api: 'generateNibiruWallet', explorer: 'https://explorer.nibiru.fi/nibiru-1/account/' },
  { name: 'Dymension', symbol: 'DYM', icon: '🎯', api: 'generateDymensionWallet', explorer: 'https://www.mintscan.io/dymension/account/' },
  { name: 'ImmutableX', symbol: 'IMX', icon: '🎨', api: 'generateImmutableXWallet', explorer: 'https://immutascan.io/address/' },
  { name: 'Loopring', symbol: 'LRC', icon: '⭕', api: 'generateLoopringWallet', explorer: 'https://etherscan.io/address/' },
  { name: 'Ultra', symbol: 'UOS', icon: '🎮', api: 'generateUltraWallet', explorer: 'https://explorer.ultra.io/account/' },
...
const template = (network) => `<script setup>
...
async function fetchWallets() {
  ...
  try {
    ...
    console.error('[${network.name}] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/${network.api}', {
      method: 'POST',
      body: {
        userID: userID.value,
        walletName: \`${network.name} Wallet \${Date.now()}\`
      }
    });
    ...
    console.error('[${network.name}] Error generating wallet:', error);
...
<template>
  <div class="network-page">
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <div class="title-section">
          ...
          <div>
            <h1>${network.name}</h1>
            <p class="subtitle">Manage your ${network.symbol} wallets and addresses</p>
          </div>
        </div>
    ...
    <div class="stats-grid">
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          <span class="stat-icon">💎</span>
          <div class="stat-info">
            <span class="stat-label">Total ${network.symbol}</span>
    ...
    <!-- Wallets List -->
    <n-card title="💼 Your ${network.name} Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        <span style="font-size: 48px;">${network.icon}</span>
        <h3 style="margin-top: 16px;">No ${network.name} Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first ${network.name} wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
        ...
        >
          <div class="wallet-header">
            <div class="wallet-title">
              <span class="wallet-emoji">${network.icon}</span>
              <div>
          ...
          <div class="wallet-info">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{ wallet.balance || '0' }} ${network.symbol}</span>
            </div>
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(\`${network.explorer}\${wallet.address}\`, '_blank')">
              🔍 View on Explorer
            </n-button>
          </div>
        </n-card>
    ...
    <n-modal
      ...
      preset="card"
      title="${network.icon} Wallet Details"
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            ${network.name} ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
          ...
          <n-descriptions-item label="Balance">
            {{ selectedWallet.balance || '0' }} ${network.symbol}
          </n-descriptions-item>
        ...
        <div style="display: flex; gap: 12px;">
          <n-button type="primary" block @click="window.open(\`${network.explorer}\${selectedWallet.address}\`, '_blank')">
            🔍 View on Explorer
          </n-button>
        </div>
      </div>
...
networks.forEach(network => {
  const fileName = network.name.replace(/\s+/g, '') + '.vue';
  ...
  if (fs.existsSync(filePath)) {
    ...
    fs.writeFileSync(filePath, template(network), 'utf8');
