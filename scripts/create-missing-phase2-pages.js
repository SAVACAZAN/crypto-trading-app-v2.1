/**
 * Create missing Phase 2 network pages
 */
...
const networks = [
  { key: 'zkSync', name: 'zkSync Era', symbol: 'ETH', chainId: '324', icon: '⚡', type: 'Layer 2', description: 'zkRollup scaling solution' },
  { key: 'Fantom', name: 'Fantom', symbol: 'FTM', chainId: '250', icon: '👻', type: 'Layer 1', description: 'High-speed smart contract platform' },
  { key: 'Linea', name: 'Linea', symbol: 'ETH', chainId: '59144', icon: '🔷', type: 'Layer 2', description: 'ConsenSys zkEVM' },
  { key: 'Scroll', name: 'Scroll', symbol: 'ETH', chainId: '534352', icon: '📜', type: 'Layer 2', description: 'zkEVM Layer 2' },
  { key: 'Blast', name: 'Blast', symbol: 'ETH', chainId: '81457', icon: '💥', type: 'Layer 2', description: 'Native yield Layer 2' },
  { key: 'Mantle', name: 'Mantle', symbol: 'MNT', chainId: '5000', icon: '🔺', type: 'Layer 2', description: 'Modular Layer 2 network' },
  { key: 'Cronos', name: 'Cronos', symbol: 'CRO', chainId: '25', icon: '💠', type: 'EVM Chain', description: 'Crypto.com blockchain' },
  { key: 'Harmony', name: 'Harmony', symbol: 'ONE', chainId: '1666600000', icon: '🎵', type: 'Layer 1', description: 'Fast and secure blockchain' },
  { key: 'Moonbeam', name: 'Moonbeam', symbol: 'GLMR', chainId: '1284', icon: '🌙', type: 'Parachain', description: 'Polkadot smart contract platform' },
  { key: 'Gnosis', name: 'Gnosis Chain', symbol: 'xDAI', chainId: '100', icon: '🦉', type: 'Sidechain', description: 'Community-owned network' }
];

function generatePageTemplate(network) {
  return `<template>
        ...
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <div>
              ...
              <p style="margin: 0; color: #666; font-size: 14px;">${network.type} - ${network.description}</p>
            </div>
          </div>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <div style="padding: 20px;">
            ...
            <n-descriptions bordered :column="2">
              <n-descriptions-item label="Network Name">${network.name}</n-descriptions-item>
              <n-descriptions-item label="Chain ID">${network.chainId}</n-descriptions-item>
              <n-descriptions-item label="Native Currency">${network.symbol}</n-descriptions-item>
              <n-descriptions-item label="Network Type">${network.type}</n-descriptions-item>
            </n-descriptions>
...
networks.forEach(network => {
  ...
  const content = generatePageTemplate(network);

  fs.writeFileSync(filepath, content, 'utf-8');
  console.log(`✓ Created ${filename}`);
});

console.log(`\nSuccessfully created ${networks.length} network pages!`);
