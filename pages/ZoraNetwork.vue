  ...
  <div class="zora-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #000000, #333333); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🎨
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Zora Network</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="7777777">
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Zora Network Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Zora Network wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Zora Network Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🎨 About Zora Network">
              <p style="line-height: 1.8; color: #666;">
                Zora Network is an Ethereum Layer 2 built on the OP Stack, designed specifically for NFT creators and collectors. It provides a fast, affordable platform for minting and trading NFTs.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.zora.energy" target="_blank">https://rpc.zora.energy</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.zora.energy" target="_blank">https://explorer.zora.energy</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>NFT Optimized:</strong> Built for digital art</li>
                <li>💰 <strong>Low Minting Costs:</strong> Affordable NFT creation</li>
                <li>⚡ <strong>Fast Transactions:</strong> Quick NFT operations</li>
                <li>🔐 <strong>Creator Focused:</strong> Tools for artists</li>
                <li>🛠️ <strong>OP Stack:</strong> Optimism-based infrastructure</li>
                <li>🌍 <strong>NFT Ecosystem:</strong> Vibrant creator community</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://zora.co')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.zora.energy')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://bridge.zora.energy')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.zora.energy')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[Zora Network] Load error:', error);
    message.error('Failed to load Zora Network wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Zora',
        walletName: `Zora Network Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Zora Network wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Zora Network] Generate error:', error);
...
function openExplorer(address) {
  window.open(`https://explorer.zora.energy/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(000000, 0.05), rgba(333333, 0.05));
}
