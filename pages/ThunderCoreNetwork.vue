<template>
  <div class="thundercore-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #FFC000, #D9A000); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              ⚡
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">ThunderCore</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="108">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="TT">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No ThunderCore Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first ThunderCore wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'ThunderCore Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="⚡ About ThunderCore">
              <p style="line-height: 1.8; color: #666;">
                ThunderCore is an EVM-compatible public blockchain with fast transaction speeds and low gas costs, designed for mainstream adoption.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://mainnet-rpc.thundercore.com" target="_blank">https://mainnet-rpc.thundercore.com</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://viewblock.io/thundercore" target="_blank">https://viewblock.io/thundercore</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>PaLa Consensus:</strong> Fast and secure</li>
                <li>💰 <strong>Low Fees:</strong> $0.00001 per tx</li>
                <li>⚡ <strong>1-2 Second Blocks:</strong> Fast finality</li>
                <li>🔐 <strong>Secure:</strong> Battle-tested</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Ethereum support</li>
                <li>🌍 <strong>User Friendly:</strong> Easy onboarding</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.thundercore.com')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://viewblock.io/thundercore')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://www.thundercore.com/thundercore-hub')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.thundercore.com')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load ThunderCore wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'ThunderCore',
        walletName: `ThunderCore Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('ThunderCore wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://viewblock.io/thundercore/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(FFC000, 0.05), rgba(D9A000, 0.05));
}
