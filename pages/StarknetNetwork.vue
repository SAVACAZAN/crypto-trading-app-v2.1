...
async function fetchWallets() {
  ...
  try {
    ...
    console.error('[Starknet] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateStarknetWallet', {
      ...
      body: {
        userID: userID.value,
        walletName: `Starknet Wallet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('Starknet wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Starknet] Error generating wallet:', error);
...
<template>
  <div class="cardano-network-page">
    ...
    <!-- Wallets List -->
    <n-card title="💼 Your Starknet Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No Starknet Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first Starknet wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
      ...
      <div v-else class="wallets-grid">
        ...
        >
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(`https://voyager.online/address/${wallet.address}`, '_blank')">
              🔍 View on StarknetScan
            </n-button>
          </div>
        </n-card>
      </div>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            Starknet ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
        ...
        <div style="display: flex; gap: 12px;">
          <n-button type="primary" block @click="window.open(`https://voyager.online/address/${selectedWallet.address}`, '_blank')">
            🔍 View on StarknetScan
          </n-button>
        </div>
      </div>
