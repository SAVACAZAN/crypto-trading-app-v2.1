<template>
  <div class="efinity-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #D946EF, #A21CAF); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              💎
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Efinity</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="1110">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="EFI">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Efinity Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Efinity wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Efinity Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="💎 About Efinity">
              <p style="line-height: 1.8; color: #666;">
                Efinity is a next-generation blockchain for NFTs built on Polkadot by Enjin. It provides advanced NFT functionalities with cross-chain capabilities.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.efinity.io" target="_blank">https://rpc.efinity.io</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.efinity.io" target="_blank">https://explorer.efinity.io</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>NFT Focused:</strong> Built for digital assets</li>
                <li>💰 <strong>Fuel Tanks:</strong> Sponsored transactions</li>
                <li>⚡ <strong>Paratoken:</strong> Advanced NFT standard</li>
                <li>🔐 <strong>Polkadot Security:</strong> Shared security</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Cross-VM support</li>
                <li>🌍 <strong>Enjin Ecosystem:</strong> Large community</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://efinity.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.efinity.io')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://enjin.io/products/wallet')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://nft.io')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Efinity wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Efinity',
        walletName: `Efinity Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Efinity wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.efinity.io/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(D946EF, 0.05), rgba(A21CAF, 0.05));
}
