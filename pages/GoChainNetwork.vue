<template>
  <div class="gochain-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #4AC656, #3BA347); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🟢
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">GoChain</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="60">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="GO">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No GoChain Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first GoChain wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'GoChain Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🟢 About GoChain">
              <p style="line-height: 1.8; color: #666;">
                GoChain is a scalable, high-performance, and low-cost blockchain that is fully compatible with Ethereum. It uses Proof of Reputation consensus.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.gochain.io" target="_blank">https://rpc.gochain.io</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.gochain.io" target="_blank">https://explorer.gochain.io</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>PoR Consensus:</strong> Proof of Reputation</li>
                <li>💰 <strong>1000x Cheaper:</strong> Than Ethereum</li>
                <li>⚡ <strong>10x Faster:</strong> Higher throughput</li>
                <li>🔐 <strong>Green:</strong> Energy efficient</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Ethereum support</li>
                <li>🌍 <strong>Enterprise Ready:</strong> Business focused</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://gochain.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.gochain.io')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.gochain.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.gochain.io')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load GoChain wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'GoChain',
        walletName: `GoChain Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('GoChain wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.gochain.io/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(4AC656, 0.05), rgba(3BA347, 0.05));
}
