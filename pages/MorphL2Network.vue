<template>
  <div class="morphl2-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #4F46E5, #3730A3); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🔀
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Morph L2</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="2818">
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Morph L2 Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Morph L2 wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Morph L2 Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🔀 About Morph L2">
              <p style="line-height: 1.8; color: #666;">
                Morph is a fully EVM-compatible Layer 2 rollup built to scale Ethereum with optimized performance and developer-friendly tools.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.morphl2.io" target="_blank">https://rpc.morphl2.io</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.morphl2.io" target="_blank">https://explorer.morphl2.io</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Optimistic Rollup:</strong> Ethereum Layer 2</li>
                <li>💰 <strong>Cost Efficient:</strong> Low transaction fees</li>
                <li>⚡ <strong>Fast Processing:</strong> Quick confirmations</li>
                <li>🔐 <strong>Ethereum Security:</strong> L1 security guarantees</li>
                <li>🛠️ <strong>Dev Friendly:</strong> Easy integration</li>
                <li>🌍 <strong>Full EVM:</strong> Complete compatibility</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://morphl2.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.morphl2.io')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://bridge.morphl2.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.morphl2.io')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Morph L2 wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'MorphL2',
        walletName: `Morph L2 Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Morph L2 wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.morphl2.io/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(4F46E5, 0.05), rgba(3730A3, 0.05));
}
