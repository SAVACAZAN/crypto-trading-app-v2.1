  ...
  <div class="metis-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #00D4D4, #00A0A0); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🌊
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Metis</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="1088">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="METIS">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Metis Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Metis wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Metis Wallet ' + (index + 1)}`" hoverable>
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="Balance">
                  <div>
                    <div>{{ wallet.balance || '0' }} METIS</div>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🌊 About Metis">
              <p style="line-height: 1.8; color: #666;">
                Metis is an Ethereum Layer 2 Optimistic Rollup platform focused on solving Ethereum scalability issues. It features decentralized sequencers and provides infrastructure for DAOs and dApps.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://andromeda.metis.io/?owner=1088" target="_blank">https://andromeda.metis.io/?owner=1088</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://andromeda-explorer.metis.io" target="_blank">https://andromeda-explorer.metis.io</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Optimistic Rollup:</strong> Scalable Layer 2 solution</li>
                <li>💰 <strong>Low Cost:</strong> Minimal transaction fees</li>
                <li>⚡ <strong>Fast Transactions:</strong> Quick finality</li>
                <li>🔐 <strong>Decentralized Sequencer:</strong> No single point of failure</li>
                <li>🛠️ <strong>EVM Equivalent:</strong> Easy migration from Ethereum</li>
                <li>🌍 <strong>DAO Focus:</strong> Built for decentralized organizations</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.metis.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://andromeda-explorer.metis.io')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://bridge.metis.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.metis.io')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Metis wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Metis',
        walletName: `Metis Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Metis wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://andromeda-explorer.metis.io/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(00D4D4, 0.05), rgba(00A0A0, 0.05));
}
