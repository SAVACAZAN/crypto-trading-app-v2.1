<template>
  <div class="smartbch-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #8DC351, #6FA936); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              💚
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">SmartBCH</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Sidechain / Bitcoin Cash</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="10000">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Sidechain / Bitcoin Cash">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="BCH">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No SmartBCH Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first SmartBCH wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'SmartBCH Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="💚 About SmartBCH">
              <p style="line-height: 1.8; color: #666;">
                SmartBCH is an EVM-compatible sidechain for Bitcoin Cash, bringing DeFi and smart contracts to the BCH ecosystem.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://smartbch.greyh.at" target="_blank">https://smartbch.greyh.at</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://www.smartscan.cash" target="_blank">https://www.smartscan.cash</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>BCH Sidechain:</strong> Bitcoin Cash powered</li>
                <li>💰 <strong>Low Fees:</strong> Affordable transactions</li>
                <li>⚡ <strong>Fast Blocks:</strong> 6-second finality</li>
                <li>🔐 <strong>SHA-Gate:</strong> Decentralized bridge</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Solidity support</li>
                <li>🌍 <strong>DeFi Ready:</strong> DEXs and more</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://smartbch.org')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://www.smartscan.cash')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://metamask.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://app.hop.ag')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load SmartBCH wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'SmartBCH',
        walletName: `SmartBCH Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('SmartBCH wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://www.smartscan.cash/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(8DC351, 0.05), rgba(6FA936, 0.05));
}
