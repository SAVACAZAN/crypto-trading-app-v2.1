<template>
  <div class="oasisemerald-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #0092F6, #0074C8); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🌹
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Oasis Emerald</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 1 / Privacy</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="42262">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 1 / Privacy">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="ROSE">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Oasis Emerald Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Oasis Emerald wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Oasis Emerald Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🌹 About Oasis Emerald">
              <p style="line-height: 1.8; color: #666;">
                Oasis Emerald is an EVM-compatible ParaTime on the Oasis Network, offering scalable, privacy-enabled DeFi with instant finality.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://emerald.oasis.dev" target="_blank">https://emerald.oasis.dev</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.emerald.oasis.dev" target="_blank">https://explorer.emerald.oasis.dev</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Privacy First:</strong> Confidential compute</li>
                <li>💰 <strong>Low Fees:</strong> 99% lower than Ethereum</li>
                <li>⚡ <strong>Instant Finality:</strong> 6-second blocks</li>
                <li>🔐 <strong>Secure Enclaves:</strong> TEE technology</li>
            ...
            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://oasisprotocol.org')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.emerald.oasis.dev')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.oasis.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.oasis.io')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Oasis Emerald wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        ...
        walletName: `Oasis Emerald Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Oasis Emerald wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.emerald.oasis.dev/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(0092F6, 0.05), rgba(0074C8, 0.05));
}
