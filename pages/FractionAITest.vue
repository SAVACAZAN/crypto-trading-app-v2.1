...
async function fetchWallets() {
  ...
  try {
    ...
    if (response.success) {
      ...
      console.log(`[Fraction AI Test] Loaded ${wallets.value.length} wallets`);
    } else {
      window.$message?.error(response.message || 'Failed to fetch wallets');
    }
  } catch (error) {
    console.error('[Fraction AI Test] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/TestNets/generateFractionAIWallet', {
      method: 'POST',
      body: {
        userID: userID.value,
        walletName: `Fraction AI Testnet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('Fraction AI testnet wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Fraction AI Test] Error generating wallet:', error);
...
<template>
  <div class="fractionai-test-page">
    <!-- Header -->
    <div class="page-header">
      <div class="header-content">
        <div class="title-section">
          ...
          <div>
            <h1>Fraction AI Testnet</h1>
            <p class="subtitle">AI-focused testnet on Sepolia with airdrop potential</p>
          </div>
        </div>
    ...
    <n-alert type="info" style="margin-bottom: 24px;">
      ...
      <strong>Fraction AI Network</strong><br>
      AI-focused testnet deployed on Sepolia. Complete AI tasks, interact with the dApp, and potentially qualify for future airdrops.
      <div style="margin-top: 8px; display: flex; gap: 8px;">
        <n-button size="small" type="primary" @click="window.open('https://fractionai.xyz/testnet', '_blank')">
          🚀 Testnet Portal
        </n-button>
        <n-button size="small" @click="window.open('https://sepoliafaucet.com', '_blank')">
          💧 Sepolia Faucet
        </n-button>
        <n-button size="small" @click="window.open('https://fractionai.xyz', '_blank')">
          🌐 Website
        </n-button>
      </div>
    </n-alert>
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          ...
          <div class="stat-info">
            ...
            <span class="stat-value">AI/EVM</span>
          </div>
        </div>
      </n-card>
    </div>
    ...
    <n-card title="📝 Testnet Tasks" style="margin-bottom: 24px;">
      <n-checkbox-group>
        <div style="display: flex; flex-direction: column; gap: 12px;">
          <n-checkbox value="1">Connect to Sepolia network</n-checkbox>
          <n-checkbox value="2">Get Sepolia ETH from faucet</n-checkbox>
          <n-checkbox value="3">Complete AI tasks</n-checkbox>
          <n-checkbox value="4">Interact with Fraction AI dApp</n-checkbox>
        </div>
      </n-checkbox-group>
    </n-card>

    <!-- Wallets List -->
    <n-card title="💼 Your Fraction AI Testnet Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No Fraction AI Testnet Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first Fraction AI testnet wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            Fraction AI Testnet ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
