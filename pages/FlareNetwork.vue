  ...
  <div class="flare-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #E52142, #B81A36); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🔥
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Flare Network</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 1 / Oracle</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="14">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 1 / Oracle">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="FLR">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Flare Network Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Flare Network wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Flare Network Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🔥 About Flare Network">
              <p style="line-height: 1.8; color: #666;">
                Flare is a blockchain for data, providing decentralized access to high-integrity data from other chains and the internet. Features the Flare Time Series Oracle.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://flare-api.flare.network/ext/C/rpc" target="_blank">https://flare-api.flare.network/ext/C/rpc</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://flare-explorer.flare.network" target="_blank">https://flare-explorer.flare.network</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Data Layer:</strong> Blockchain for data</li>
                <li>💰 <strong>FTSO:</strong> Native price oracle</li>
                <li>⚡ <strong>State Connector:</strong> Trustless bridging</li>
                <li>🔐 <strong>FBA Consensus:</strong> Avalanche-based</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling</li>
                <li>🌍 <strong>Cross-Chain:</strong> Multi-chain data</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://flare.network')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://flare-explorer.flare.network')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://bifrostwallet.com')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://portal.flare.network')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[Flare Network] Load error:', error);
    message.error('Failed to load Flare Network wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Flare',
        walletName: `Flare Network Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Flare Network wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Flare Network] Generate error:', error);
...
function openExplorer(address) {
  window.open(`https://flare-explorer.flare.network/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(E52142, 0.05), rgba(B81A36, 0.05));
}
