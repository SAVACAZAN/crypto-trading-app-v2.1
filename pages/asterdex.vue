<template>
  <div class="asterdex-page">
    <n-space vertical :size="20">
      <!-- Header -->
      <n-card style="background: linear-gradient(135deg, #1a1a1a 0%, #2d1f0d 100%); border: 2px solid #ff6600;">
        <template #header>
          <div style="display: flex; align-items: center; gap: 16px;">
            <div style="padding: 12px; background: linear-gradient(135deg, #ff6600 0%, #ff8800 100%); border-radius: 12px;">
            ...
            <div>
              <h1 style="margin: 0; font-size: 28px; color: #ff6600;">⭐ Asterdex Exchange</h1>
      ...
      <!-- API Key Info -->
      <n-card v-if="selectedApiKey !== null" style="border: 1px solid #333; background: rgba(255, 102, 0, 0.05);">
        <n-space :size="12" align="center">
          <div style="padding: 8px; background: #ff6600; border-radius: 8px;">
          ...
          <div>
            <div style="font-size: 14px; color: #ff6600; font-weight: 700;">
      ...
      <!-- Balance Section -->
      <n-card title="💰 Account Balance" style="border: 1px solid #ff6600;">
      ...
      <!-- Markets Section -->
      <n-card title="📊 Available Markets" style="border: 1px solid #ff6600;">
...
const balanceColumns = [
  {
    ...
    key: 'asset',
    width: 120,
    render: (row) => h('span', { style: 'font-weight: 700; color: #ff6600;' }, row.asset)
  },
  {
    title: 'Free',
    key: 'free',
...
const marketColumns = [
  {
    ...
    key: 'symbol',
    width: 150,
    render: (row) => h('span', { style: 'font-weight: 700; color: #ff6600;' }, row.symbol)
  },
  {
    title: 'Base',
    key: 'base',
...
async function fetchApiKeys() {
  try {
    ...
    console.error('Failed to fetch Asterdex API keys:', error);
    notification.error({
      content: 'Error',
...
async function fetchBalance() {
  ...
  try {
    ...
    const response = await $fetch('/api/v1/asterdex/balance', {
      method: 'POST',
      body: {
...
async function fetchMarkets() {
  ...
  try {
    const response = await $fetch('/api/v1/asterdex/markets');
