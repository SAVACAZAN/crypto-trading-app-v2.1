...
async function fetchWallets() {
  ...
  try {
    ...
    if (response.success) {
      ...
      console.log(`[Ripple] Loaded ${wallets.value.length} wallets`);
    } else {
      window.$message?.error(response.message || 'Failed to fetch wallets');
    }
  } catch (error) {
    console.error('[Ripple] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateRippleWallet', {
      ...
      body: {
        userID: userID.value,
        walletName: `Ripple Wallet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('Ripple wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Ripple] Error generating wallet:', error);
...
<template>
  <div class="ripple-network-page">
      ...
      <div class="header-content">
        ...
        <div class="title-section">
          ...
          <div>
            ...
            <p class="subtitle">Enterprise blockchain for global payments</p>
          </div>
        </div>
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          ...
          <div class="stat-info">
            ...
            <span class="stat-value">{{ totalBalance.toFixed(4) }} XRP</span>
          </div>
        </div>
      </n-card>
    ...
    <!-- Wallets List -->
    <n-card title="💼 Your Ripple Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No Ripple Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first Ripple wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
        ...
        >
          ...
          <div class="wallet-info">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{ wallet.balance || '0' }} XRP</span>
            </div>
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(`https://livenet.xrpl.org/accounts/${wallet.address}`, '_blank')">
              🔍 Explorer
            </n-button>
          </div>
        </n-card>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            Ripple ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
        ...
        <div style="display: flex; flex-direction: column; gap: 8px;">
          <n-button type="primary" block @click="window.open(`https://livenet.xrpl.org/accounts/${selectedWallet.address}`, '_blank')">
            🔍 View on Explorer
          </n-button>
          <n-button block @click="window.open('https://ripple.com', '_blank')">
            🌐 Ripple Website
          </n-button>
        </div>
