<template>
  <div class="songbird-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #FFC107, #FFA000); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🐦
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Songbird</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 1 / Canary</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="19">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 1 / Canary">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="SGB">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Songbird Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Songbird wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Songbird Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🐦 About Songbird">
              <p style="line-height: 1.8; color: #666;">
                Songbird is the canary network for Flare, testing new features with real economic value before deployment to Flare Network.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://songbird-api.flare.network/ext/C/rpc" target="_blank">https://songbird-api.flare.network/ext/C/rpc</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://songbird-explorer.flare.network" target="_blank">https://songbird-explorer.flare.network</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Canary Network:</strong> Test with value</li>
                <li>💰 <strong>FTSO:</strong> Time series oracle</li>
                <li>⚡ <strong>Fast:</strong> Quick finality</li>
                <li>🔐 <strong>Battle Testing:</strong> Real value</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Ethereum support</li>
                <li>🌍 <strong>Innovation:</strong> Latest features first</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://flare.network/songbird')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://songbird-explorer.flare.network')" block>
                  🔍 Block Explorer
                </n-button>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Songbird wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Songbird',
        walletName: `Songbird Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Songbird wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://songbird-explorer.flare.network/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(FFC107, 0.05), rgba(FFA000, 0.05));
}
