<template>
  <div class="polygon-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #8247E5, #A855F7); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px; color: white;">
              ⬢
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Polygon Network</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 2 Scaling Solution</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="137">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="POL">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Polygon Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Polygon wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Polygon Wallet ' + (index + 1)}`" hoverable>
          ...
          <n-space vertical :size="16">
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://polygon-rpc.com" target="_blank">https://polygon-rpc.com</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://polygonscan.com" target="_blank">https://polygonscan.com</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>High Throughput:</strong> Up to 65,000 TPS on a single sidechain</li>
                <li>💰 <strong>Low Fees:</strong> Transaction costs as low as $0.01</li>
                <li>⚡ <strong>Fast Finality:</strong> ~2 second block time</li>
                <li>🔐 <strong>Ethereum Security:</strong> Secured by Ethereum's validator set</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Ethereum tooling support</li>
                <li>🌍 <strong>Vibrant Ecosystem:</strong> 19,000+ dApps and growing</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://polygon.technology')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://polygonscan.com')" block>
                  🔍 PolygonScan Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.polygon.technology')" block>
                  👛 Polygon Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://wallet.polygon.technology/polygon/bridge')" block>
                  🌉 Polygon Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[Polygon] Load error:', error);
    message.error('Failed to load Polygon wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Polygon',
        walletName: `Polygon Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Polygon wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Polygon] Generate error:', error);
...
.header-card {
  background: linear-gradient(135deg, rgba(130, 71, 229, 0.05), rgba(168, 85, 247, 0.05));
}
