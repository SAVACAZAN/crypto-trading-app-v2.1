<template>
  <div class="arbitrumnova-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #28A0F0, #1E7FC0); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🎮
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Arbitrum Nova</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 2 / AnyTrust</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="42170">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 2 / AnyTrust">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Arbitrum Nova Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Arbitrum Nova wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Arbitrum Nova Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🎮 About Arbitrum Nova">
              <p style="line-height: 1.8; color: #666;">
                Arbitrum Nova is an AnyTrust chain optimized for gaming and social applications. It offers ultra-low transaction costs while maintaining high security through a Data Availability Committee.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://nova.arbitrum.io/rpc" target="_blank">https://nova.arbitrum.io/rpc</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://nova.arbiscan.io" target="_blank">https://nova.arbiscan.io</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>AnyTrust Technology:</strong> Optimized for gaming</li>
                <li>💰 <strong>Ultra Low Fees:</strong> Minimal transaction costs</li>
                <li>⚡ <strong>High Performance:</strong> Fast transactions</li>
                <li>🔐 <strong>Secure:</strong> Data Availability Committee</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Ethereum compatibility</li>
                <li>🌍 <strong>Gaming Focused:</strong> Built for Web3 games</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://nova.arbitrum.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://nova.arbiscan.io')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://bridge.arbitrum.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.arbitrum.io/?destinationChain=arbitrum-nova')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Arbitrum Nova wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        ...
        walletName: `Arbitrum Nova Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Arbitrum Nova wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://nova.arbiscan.io/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(28A0F0, 0.05), rgba(1E7FC0, 0.05));
}
