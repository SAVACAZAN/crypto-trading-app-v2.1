<template>
  <div class="polygonzkevm-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #8247E5, #A855F7); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🔷
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Polygon zkEVM</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="1101">
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Polygon zkEVM Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Polygon zkEVM wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Polygon zkEVM Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🔷 About Polygon zkEVM">
              <p style="line-height: 1.8; color: #666;">
                Polygon zkEVM is a decentralized Ethereum Layer 2 scalability solution that uses cryptographic zero-knowledge proofs to offer validity and fast finality to off-chain transaction computation. It provides EVM equivalence with enhanced scalability and lower costs.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://zkevm-rpc.com" target="_blank">https://zkevm-rpc.com</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://zkevm.polygonscan.com" target="_blank">https://zkevm.polygonscan.com</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>ZK-Rollup Technology:</strong> Advanced zero-knowledge proofs</li>
                <li>💰 <strong>Low Fees:</strong> Significantly cheaper than Ethereum L1</li>
                <li>⚡ <strong>Fast Finality:</strong> Quick transaction confirmation</li>
                <li>🔐 <strong>Ethereum Security:</strong> Inherits Ethereum security</li>
                <li>🛠️ <strong>EVM Equivalent:</strong> Full EVM compatibility</li>
                <li>🌍 <strong>Polygon Ecosystem:</strong> Part of the Polygon suite</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://polygon.technology/polygon-zkevm')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://zkevm.polygonscan.com')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.polygon.technology')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.polygon.technology')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Polygon zkEVM wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        ...
        walletName: `Polygon zkEVM Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Polygon zkEVM wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://zkevm.polygonscan.com/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(8247E5, 0.05), rgba(A855F7, 0.05));
}
