  ...
  <div class="kava-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #FF433E, #D9362F); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🔴
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Kava</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="2222">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="KAVA">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Kava Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Kava wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Kava Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🔴 About Kava">
              <p style="line-height: 1.8; color: #666;">
                Kava is a Layer 1 blockchain combining the speed of Cosmos with the developer power of Ethereum. It provides a secure platform for DeFi applications.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://evm.kava.io" target="_blank">https://evm.kava.io</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.kava.io" target="_blank">https://explorer.kava.io</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Cosmos + Ethereum:</strong> Best of both</li>
                <li>💰 <strong>DeFi Platform:</strong> Lending, staking, more</li>
                <li>⚡ <strong>Fast Finality:</strong> 6-second blocks</li>
                <li>🔐 <strong>Secure:</strong> Battle-tested</li>
                <li>🛠️ <strong>EVM Co-Chain:</strong> Full Solidity support</li>
                <li>🌍 <strong>Cross-Chain:</strong> IBC connectivity</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.kava.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.kava.io')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://app.kava.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://app.kava.io/bridge')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Kava wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Kava',
        walletName: `Kava Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Kava wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.kava.io/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(FF433E, 0.05), rgba(D9362F, 0.05));
}
