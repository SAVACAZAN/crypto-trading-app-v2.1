...
async function fetchWallets() {
  ...
  try {
    ...
    console.error('[Nibiru] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateNibiruWallet', {
      ...
      body: {
        userID: userID.value,
        walletName: `Nibiru Wallet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('Nibiru wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Nibiru] Error generating wallet:', error);
...
<template>
  <div class="network-page">
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <div class="title-section">
          ...
          <div>
            <h1>Nibiru</h1>
            <p class="subtitle">Manage your NIBI wallets and addresses</p>
          </div>
        </div>
    ...
    <div class="stats-grid">
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          <span class="stat-icon">💎</span>
          <div class="stat-info">
            <span class="stat-label">Total NIBI</span>
    ...
    <!-- Wallets List -->
    <n-card title="💼 Your Nibiru Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No Nibiru Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first Nibiru wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
        ...
        >
          ...
          <div class="wallet-info">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{ wallet.balance || '0' }} NIBI</span>
            </div>
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(`https://explorer.nibiru.fi/nibiru-1/account/${wallet.address}`, '_blank')">
              🔍 View on Explorer
            </n-button>
          </div>
        </n-card>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            Nibiru ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
        ...
        <div style="display: flex; gap: 12px;">
          <n-button type="primary" block @click="window.open(`https://explorer.nibiru.fi/nibiru-1/account/${selectedWallet.address}`, '_blank')">
            🔍 View on Explorer
          </n-button>
        </div>
      </div>
