  ...
  <div class="hedera-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #000000, #2A2A2A); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              ⓗ
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Hedera</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 1 / Hashgraph</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="295">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 1 / Hashgraph">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="HBAR">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Hedera Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Hedera wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Hedera Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="ⓗ About Hedera">
              <p style="line-height: 1.8; color: #666;">
                Hedera is a public distributed ledger using the Hashgraph consensus algorithm. It provides fast, fair, and secure transactions with enterprise-grade governance.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://mainnet.hashio.io/api" target="_blank">https://mainnet.hashio.io/api</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://hashscan.io" target="_blank">https://hashscan.io</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Hashgraph:</strong> Advanced consensus</li>
                <li>💰 <strong>Low Fees:</strong> Predictable costs ($0.0001)</li>
                <li>⚡ <strong>Ultra Fast:</strong> 10,000+ TPS</li>
                <li>🔐 <strong>Enterprise Grade:</strong> Council governance</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Solidity support</li>
                <li>🌍 <strong>Sustainable:</strong> Carbon-negative network</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://hedera.com')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://hashscan.io')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://www.hashpack.app')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://www.hashport.network')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Hedera wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Hedera',
        walletName: `Hedera Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Hedera wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://hashscan.io/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(000000, 0.05), rgba(2A2A2A, 0.05));
}
