<template>
  <div class="worldchain-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <div>
              <h2 style="margin: 0; font-size: 24px;">World Chain</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="480">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="WLD">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No World Chain Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first World Chain wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'World Chain Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🌍 About World Chain">
              <p style="line-height: 1.8; color: #666;">
                World Chain is an Ethereum Layer 2 built for humans, powered by Worldcoin. It offers priority blockspace for verified humans and gas-free transactions.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://worldchain-mainnet.g.alchemy.com/public" target="_blank">https://worldchain-mainnet.g.alchemy.com/public</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://worldscan.org" target="_blank">https://worldscan.org</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Human Centric:</strong> Built for people</li>
                <li>💰 <strong>Gas-Free:</strong> For verified humans</li>
                <li>⚡ <strong>OP Stack:</strong> Optimism-based</li>
                <li>🔐 <strong>World ID:</strong> Proof of personhood</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling</li>
                <li>🌍 <strong>Global:</strong> Worldcoin integration</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://worldcoin.org/world-chain')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://worldscan.org')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://world.org')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.worldchain.org')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load World Chain wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'WorldChain',
        walletName: `World Chain Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('World Chain wallet generated successfully!');
      await loadWallets();
    } else {
