  ...
  <div class="canto-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #06FC99, #05D981); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🎵
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Canto</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="7700">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="CANTO">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Canto Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Canto wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Canto Wallet ' + (index + 1)}`" hoverable>
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="Balance">
                  <div>
                    <div>{{ wallet.balance || '0' }} CANTO</div>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🎵 About Canto">
              <p style="line-height: 1.8; color: #666;">
                Canto is a permissionless Layer 1 blockchain built to deliver on the promise of DeFi. It offers Free Public Infrastructure for liquidity, lending, and stablecoins.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://canto.slingshot.finance" target="_blank">https://canto.slingshot.finance</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://tuber.build" target="_blank">https://tuber.build</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Free Infrastructure:</strong> No protocol fees</li>
                <li>💰 <strong>DeFi Focused:</strong> Built for finance</li>
                <li>⚡ <strong>Cosmos SDK:</strong> Fast and scalable</li>
                <li>🔐 <strong>Decentralized:</strong> Community owned</li>
            ...
            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://canto.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://tuber.build')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.canto.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.canto.io')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Canto wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Canto',
        walletName: `Canto Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Canto wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://tuber.build/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(06FC99, 0.05), rgba(05D981, 0.05));
}
