  ...
  <div class="ronin-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #0066FF, #0047B3); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              ⚔️
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Ronin Network</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Gaming Blockchain - Axie Infinity</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="2020">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Gaming / L1">
            <template #prefix>🎮</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="RON">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Ronin Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Ronin wallet to get started with gaming</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Ronin Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="⚔️ About Ronin">
              <p style="line-height: 1.8; color: #666;">
                Ronin is an EVM-compatible blockchain specifically designed for gaming. Originally built for Axie Infinity,
                it offers fast transactions and low fees optimized for NFT trading and gaming experiences. Ronin has become
                a leading gaming blockchain with millions of daily active users.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://api.roninchain.com/rpc" target="_blank">https://api.roninchain.com/rpc</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.roninchain.com" target="_blank">https://explorer.roninchain.com</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🎮 <strong>Gaming Optimized:</strong> Built specifically for gaming and NFTs</li>
                <li>💰 <strong>Ultra Low Fees:</strong> Gas fees as low as $0.001</li>
                <li>⚡ <strong>Fast Transactions:</strong> ~3 second block time</li>
                <li>🔐 <strong>Secure:</strong> Delegated Proof of Stake consensus</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Ethereum tooling support</li>
                <li>🌍 <strong>Gaming Ecosystem:</strong> Axie Infinity, Pixels, and more</li>
                <li>💎 <strong>NFT Focused:</strong> Optimized for NFT minting and trading</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://roninchain.com')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.roninchain.com')" block>
                  🔍 Ronin Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.roninchain.com')" block>
                  👛 Ronin Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.roninchain.com')" block>
                  🌉 Ronin Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Ronin wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Ronin',
        walletName: `Ronin Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Ronin wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.roninchain.com/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(0, 102, 255, 0.05), rgba(0, 71, 179, 0.05));
}
