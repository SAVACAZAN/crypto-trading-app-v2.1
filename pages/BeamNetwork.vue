  ...
  <div class="beam-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #A259FF, #7B3FF2); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🌟
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Beam Network</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Gaming & NFT Layer 2</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="4337">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="BEAM">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Beam Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Beam wallet for gaming</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Beam Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🌟 About Beam Network">
              <p style="line-height: 1.8; color: #666;">
                Beam is a gaming-focused Layer 2 network that combines the power of Avalanche subnets with full EVM
                compatibility. It provides a scalable infrastructure for blockchain gaming, offering low fees and
                high throughput specifically designed for gaming applications and NFT marketplaces.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.beam.network" target="_blank">https://rpc.beam.network</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.beam.network" target="_blank">https://explorer.beam.network</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🎮 <strong>Gaming Optimized:</strong> Built for blockchain gaming</li>
                <li>💰 <strong>Low Cost:</strong> Minimal transaction fees</li>
                <li>⚡ <strong>High Speed:</strong> Sub-second block times</li>
                <li>🔐 <strong>Secure:</strong> Avalanche consensus mechanism</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Solidity support</li>
                <li>🌍 <strong>Gaming Ecosystem:</strong> Multiple games and dApps</li>
                <li>💎 <strong>NFT Marketplace:</strong> Built-in NFT support</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.beam.network')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.beam.network')" block>
                  🔍 Beam Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.beam.network')" block>
                  👛 Beam Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.beam.network')" block>
                  🌉 Beam Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Beam wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Beam',
        walletName: `Beam Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Beam wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.beam.network/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(162, 89, 255, 0.05), rgba(123, 63, 242, 0.05));
}
