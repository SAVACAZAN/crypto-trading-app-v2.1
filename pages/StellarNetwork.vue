...
async function fetchWallets() {
  ...
  try {
    ...
    if (response.success) {
      ...
      console.log(`[Stellar] Loaded ${wallets.value.length} wallets`);
    } else {
      window.$message?.error(response.message || 'Failed to fetch wallets');
    }
  } catch (error) {
    console.error('[Stellar] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateStellarWallet', {
      ...
      body: {
        userID: userID.value,
        walletName: `Stellar Wallet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('Stellar wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Stellar] Error generating wallet:', error);
...
<template>
  <div class="stellar-network-page">
    <!-- Header -->
    <div class="page-header">
      <div class="header-content">
        ...
        <div class="title-section">
          ...
          <div>
            <h1>Stellar Network</h1>
            <p class="subtitle">Fast, low-cost blockchain for payments</p>
          </div>
        </div>
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          ...
          <div class="stat-info">
            ...
            <span class="stat-value">{{ totalBalance.toFixed(4) }} XLM</span>
          </div>
        </div>
      </n-card>
    ...
    <!-- Wallets List -->
    <n-card title="💼 Your Stellar Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No Stellar Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first Stellar wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
        ...
        >
          ...
          <div class="wallet-info">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{ wallet.balance || '0' }} XLM</span>
            </div>
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(`https://stellar.expert/explorer/public/account/${wallet.address}`, '_blank')">
              🔍 Explorer
            </n-button>
          </div>
        </n-card>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            Stellar ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
        ...
        <div style="display: flex; flex-direction: column; gap: 8px;">
          <n-button type="primary" block @click="window.open(`https://stellar.expert/explorer/public/account/${selectedWallet.address}`, '_blank')">
            🔍 View on Explorer
          </n-button>
          <n-button block @click="window.open('https://stellar.org', '_blank')">
            🌐 Stellar Network Website
          </n-button>
        </div>
        ...
        <n-alert type="warning" style="margin-top: 16px;">
          ...
          Never share your private key or secret seed with anyone!
        </n-alert>
      </div>
    </n-modal>
  </div>
