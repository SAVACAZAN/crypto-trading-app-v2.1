<template>
  <div class="telosevm-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #571AFF, #3D0DB3); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🔷
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Telos EVM</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="40">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="TLOS">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Telos EVM Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Telos EVM wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Telos EVM Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🔷 About Telos EVM">
              <p style="line-height: 1.8; color: #666;">
                Telos EVM is one of the fastest EVM platforms available, offering high performance and low costs for decentralized applications.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://mainnet.telos.net/evm" target="_blank">https://mainnet.telos.net/evm</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://teloscan.io" target="_blank">https://teloscan.io</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Fastest EVM:</strong> 0.5 second blocks</li>
                <li>💰 <strong>Near Zero Fees:</strong> $0.00001 per transaction</li>
                <li>⚡ <strong>10,000 TPS:</strong> High throughput</li>
                <li>🔐 <strong>Fixed Gas:</strong> Predictable costs</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Ethereum support</li>
                <li>🌍 <strong>Carbon Neutral:</strong> Eco-friendly</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.telos.net')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://teloscan.io')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.telos.net')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.telos.net')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Telos EVM wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'TelosEVM',
        walletName: `Telos EVM Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Telos EVM wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://teloscan.io/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(571AFF, 0.05), rgba(3D0DB3, 0.05));
}
