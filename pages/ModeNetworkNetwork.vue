<template>
  <div class="modenetwork-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #DFFE00, #B8CC00); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🟢
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Mode Network</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="34443">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="MODE">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Mode Network Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Mode Network wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Mode Network Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🟢 About Mode Network">
              <p style="line-height: 1.8; color: #666;">
                Mode is a Layer 2 network built on the OP Stack with a focus on DeFi and yield-generating applications. It rewards users and developers with its innovative growth mechanisms.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://mainnet.mode.network" target="_blank">https://mainnet.mode.network</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.mode.network" target="_blank">https://explorer.mode.network</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>OP Stack L2:</strong> Optimism-based</li>
                <li>💰 <strong>DeFi Focused:</strong> Built for finance</li>
                <li>⚡ <strong>Fast & Cheap:</strong> Efficient operations</li>
                <li>🔐 <strong>Ethereum Security:</strong> L1-backed</li>
                <li>🛠️ <strong>Dev Incentives:</strong> Revenue sharing</li>
                <li>🌍 <strong>Growth Rewards:</strong> User incentives</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://mode.network')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.mode.network')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://bridge.mode.network')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.mode.network')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Mode Network wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'ModeNetwork',
        walletName: `Mode Network Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Mode Network wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.mode.network/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(DFFE00, 0.05), rgba(B8CC00, 0.05));
}
