<template>
  <div class="horizeneon-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #041742, #030F2F); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🔐
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Horizen EON</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="7332">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="ZEN">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Horizen EON Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Horizen EON wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Horizen EON Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🔐 About Horizen EON">
              <p style="line-height: 1.8; color: #666;">
                Horizen EON is a privacy-focused, EVM-compatible smart contracting platform built on Horizen's zero-knowledge enabled network of blockchains.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://eon-rpc.horizenlabs.io/ethv1" target="_blank">https://eon-rpc.horizenlabs.io/ethv1</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://eon-explorer.horizenlabs.io" target="_blank">https://eon-explorer.horizenlabs.io</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Privacy Enabled:</strong> ZK technology</li>
                <li>💰 <strong>Scalable:</strong> High performance</li>
                <li>⚡ <strong>Fast Finality:</strong> Quick confirmations</li>
                <li>🔐 <strong>Secure:</strong> Enhanced privacy</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling</li>
                <li>🌍 <strong>Sidechain:</strong> Part of Horizen ecosystem</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://horizen.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://eon-explorer.horizenlabs.io')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://www.horizen.io/wallets')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.horizen.io')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Horizen EON wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'HorizenEON',
        walletName: `Horizen EON Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Horizen EON wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://eon-explorer.horizenlabs.io/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(041742, 0.05), rgba(030F2F, 0.05));
}
