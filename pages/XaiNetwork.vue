  ...
  <div class="xai-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #FF385C, #E31C5F); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🎯
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Xai Network</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 3 / Gaming</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="660279">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 3 / Gaming">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="XAI">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Xai Network Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Xai Network wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Xai Network Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🎯 About Xai Network">
              <p style="line-height: 1.8; color: #666;">
                Xai is a Layer 3 blockchain optimized for gaming, built on Arbitrum. It enables seamless Web3 gaming experiences with traditional gaming UX.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://xai-chain.net/rpc" target="_blank">https://xai-chain.net/rpc</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.xai-chain.net" target="_blank">https://explorer.xai-chain.net</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Layer 3:</strong> Built on Arbitrum</li>
                <li>💰 <strong>Gasless Gaming:</strong> No gas for players</li>
                <li>⚡ <strong>Gaming Optimized:</strong> Purpose-built for games</li>
                <li>🔐 <strong>Secure:</strong> Arbitrum-backed security</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling</li>
                <li>🌍 <strong>AAA Games:</strong> Supporting major titles</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://xai.games')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.xai-chain.net')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://sentry.xai.games')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.xai.games')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[Xai Network] Load error:', error);
    message.error('Failed to load Xai Network wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Xai',
        walletName: `Xai Network Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Xai Network wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Xai Network] Generate error:', error);
...
function openExplorer(address) {
  window.open(`https://explorer.xai-chain.net/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(FF385C, 0.05), rgba(E31C5F, 0.05));
}
