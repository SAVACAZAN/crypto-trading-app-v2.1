...
async function fetchWallets() {
  ...
  try {
    ...
    if (response.success) {
      ...
      console.log(`[Berachain Test] Loaded ${wallets.value.length} wallets`);
    } else {
      window.$message?.error(response.message || 'Failed to fetch wallets');
    }
  } catch (error) {
    console.error('[Berachain Test] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/TestNets/generateBerachainWallet', {
      method: 'POST',
      body: {
        userID: userID.value,
        walletName: `Berachain Testnet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('Berachain testnet wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Berachain Test] Error generating wallet:', error);
...
<template>
  <div class="berachain-test-page">
    ...
    <div class="page-header">
      <div class="header-content">
        <div class="title-section">
          ...
          <div>
            <h1>Berachain Testnet</h1>
            <p class="subtitle">Incentivized testnet with farming rewards</p>
          </div>
        </div>
    ...
    <n-alert type="info" style="margin-bottom: 24px;">
      ...
      <strong>Berachain Network</strong><br>
      Incentivized testnet with farming opportunities. Participate in DeFi protocols and complete farming tasks to qualify for potential airdrops.
      <div style="margin-top: 8px; display: flex; gap: 8px;">
        <n-button size="small" type="primary" @click="window.open('https://blog.berachain.com', '_blank')">
          🚀 Portal
        </n-button>
        <n-button size="small" @click="window.open('https://faucet.berachain.com', '_blank')">
          💧 Faucet
        </n-button>
        <n-button size="small" @click="window.open('https://berachain.com', '_blank')">
          🌐 Website
        </n-button>
      </div>
    </n-alert>
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          ...
          <div class="stat-info">
            ...
            <span class="stat-value">EVM</span>
          </div>
        </div>
      </n-card>
    </div>
    ...
    <!-- Task Checklist -->
    <n-card title="📝 Testnet Tasks" style="margin-bottom: 24px;">
      <n-checkbox-group>
        <div style="display: flex; flex-direction: column; gap: 12px;">
          <n-checkbox value="1">Add Berachain testnet to wallet</n-checkbox>
          <n-checkbox value="2">Get test BERA from faucet</n-checkbox>
          <n-checkbox value="3">Participate in DeFi protocols</n-checkbox>
          <n-checkbox value="4">Complete farming tasks</n-checkbox>
        </div>
      </n-checkbox-group>
    </n-card>

    <!-- Wallets List -->
    <n-card title="💼 Your Berachain Testnet Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No Berachain Testnet Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first Berachain testnet wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
        ...
        >
          ...
          <div class="wallet-info">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{ wallet.balance || '0' }} BERA</span>
            </div>
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(wallet.explorerUrl + '/address/' + wallet.address, '_blank')">
              🔍 Explorer
            </n-button>
            <n-button size="small" type="success" ghost @click="window.open(wallet.portalUrl, '_blank')">
              🚀 Portal
            </n-button>
          </div>
        </n-card>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            Berachain Testnet ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
        ...
        <div style="display: flex; flex-direction: column; gap: 8px;">
          <n-button type="primary" block @click="window.open(selectedWallet.portalUrl, '_blank')">
            🚀 Open Portal
          </n-button>
          <n-button block @click="window.open(selectedWallet.faucetUrl, '_blank')">
            💧 Get Test Tokens
          </n-button>
          <n-button block @click="window.open(selectedWallet.explorerUrl, '_blank')">
            🔍 View on Explorer
          </n-button>
        </div>
