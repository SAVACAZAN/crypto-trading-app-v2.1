<template>
  <div class="syscoin-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #0082C8, #006BA1); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🔷
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Syscoin</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="57">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="SYS">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Syscoin Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Syscoin wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Syscoin Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🔷 About Syscoin">
              <p style="line-height: 1.8; color: #666;">
                Syscoin is a Proof-of-Work blockchain merged-mined with Bitcoin, offering EVM compatibility and focused on scalability through its NEVM platform.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.syscoin.org" target="_blank">https://rpc.syscoin.org</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.syscoin.org" target="_blank">https://explorer.syscoin.org</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Bitcoin Security:</strong> Merged mining</li>
                <li>💰 <strong>Low Costs:</strong> Efficient operations</li>
                <li>⚡ <strong>Scalable:</strong> PoDA technology</li>
                <li>🔐 <strong>PoW Security:</strong> Bitcoin-backed</li>
                <li>🛠️ <strong>EVM Compatible:</strong> NEVM platform</li>
                <li>🌍 <strong>Hybrid:</strong> UTXO + EVM</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://syscoin.org')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.syscoin.org')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://syscoin.org/get-started')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.syscoin.org')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Syscoin wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Syscoin',
        walletName: `Syscoin Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Syscoin wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.syscoin.org/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(0082C8, 0.05), rgba(006BA1, 0.05));
}
