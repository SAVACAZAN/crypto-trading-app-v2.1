  ...
  <div class="astar-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #0068DB, #0052AD); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              ⭐
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Astar Network</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Parachain / Polkadot</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="592">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Parachain / Polkadot">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="ASTR">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Astar Network Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Astar Network wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Astar Network Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="⭐ About Astar Network">
              <p style="line-height: 1.8; color: #666;">
                Astar is a multi-chain smart contract platform on Polkadot that supports both EVM and WebAssembly smart contracts with cross-VM composability.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://evm.astar.network" target="_blank">https://evm.astar.network</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://astar.blockscout.com" target="_blank">https://astar.blockscout.com</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Polkadot Parachain:</strong> Shared security</li>
                <li>💰 <strong>dApp Staking:</strong> Earn by building</li>
                <li>⚡ <strong>Multi-VM:</strong> EVM + WASM</li>
                <li>🔐 <strong>Cross-Chain:</strong> XCM messaging</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Solidity support</li>
                <li>🌍 <strong>Japan Hub:</strong> Web3 gateway</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://astar.network')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://astar.blockscout.com')" block>
                  🔍 Block Explorer
                </n-button>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[Astar Network] Load error:', error);
    message.error('Failed to load Astar Network wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Astar',
        walletName: `Astar Network Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Astar Network wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Astar Network] Generate error:', error);
...
function openExplorer(address) {
  window.open(`https://astar.blockscout.com/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(0068DB, 0.05), rgba(0052AD, 0.05));
}
