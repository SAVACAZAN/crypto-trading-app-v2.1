  ...
  <div class="aurora-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #70D44B, #5AB83A); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🌌
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Aurora</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 2 / NEAR</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="1313161554">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 2 / NEAR">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Aurora Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Aurora wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Aurora Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🌌 About Aurora">
              <p style="line-height: 1.8; color: #666;">
                Aurora is an EVM-compatible blockchain built on NEAR Protocol. It provides Ethereum developers with a scalable, low-cost, and user-friendly environment while leveraging NEAR's sharding technology.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://mainnet.aurora.dev" target="_blank">https://mainnet.aurora.dev</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://aurorascan.dev" target="_blank">https://aurorascan.dev</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>NEAR-Powered:</strong> Built on NEAR Protocol</li>
                <li>💰 <strong>Low Fees:</strong> Transaction costs under $0.01</li>
                <li>⚡ <strong>High Speed:</strong> 2-second block time</li>
                <li>🔐 <strong>Secure:</strong> NEAR consensus mechanism</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling support</li>
                <li>🌍 <strong>Rainbow Bridge:</strong> Seamless Ethereum bridging</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://aurora.dev')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://aurorascan.dev')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.aurora.dev')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://rainbowbridge.app')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Aurora wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Aurora',
        walletName: `Aurora Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Aurora wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://aurorascan.dev/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(70D44B, 0.05), rgba(5AB83A, 0.05));
}
