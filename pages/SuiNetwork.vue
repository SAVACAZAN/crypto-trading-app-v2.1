...
async function fetchWallets() {
  ...
  try {
    ...
    if (response.success) {
      ...
      console.log(`[Sui] Loaded ${wallets.value.length} wallets`);
    } else {
      window.$message?.error(response.message || 'Failed to fetch wallets');
    }
  } catch (error) {
    console.error('[Sui] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateSuiWallet', {
      ...
      body: {
        userID: userID.value,
        walletName: `Sui Wallet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('Sui wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Sui] Error generating wallet:', error);
...
<template>
  <div class="sui-network-page">
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <div class="title-section">
          ...
          <div>
            <h1>Sui Network</h1>
            <p class="subtitle">High-performance Layer 1 blockchain</p>
          </div>
        </div>
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          ...
          <div class="stat-info">
            ...
            <span class="stat-value">{{ totalBalance.toFixed(4) }} SUI</span>
          </div>
        </div>
      </n-card>
    ...
    <!-- Wallets List -->
    <n-card title="💼 Your Sui Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No Sui Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first Sui wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
        ...
        >
          ...
          <div class="wallet-info">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{ wallet.balance || '0' }} SUI</span>
            </div>
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(`https://suiscan.xyz/mainnet/account/${wallet.address}`, '_blank')">
              🔍 Explorer
            </n-button>
          </div>
        </n-card>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            Sui ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
        ...
        <div style="display: flex; flex-direction: column; gap: 8px;">
          <n-button type="primary" block @click="window.open(`https://suiscan.xyz/mainnet/account/${selectedWallet.address}`, '_blank')">
            🔍 View on Explorer
          </n-button>
          <n-button block @click="window.open('https://sui.io', '_blank')">
            🌐 Sui Network Website
          </n-button>
        </div>
...
.stat-card {
  background: linear-gradient(135deg, rgba(106, 192, 255, 0.1) 0%, rgba(59, 130, 246, 0.1) 100%);
...
.wallet-card:hover {
  ...
  box-shadow: 0 4px 20px rgba(106, 192, 255, 0.2);
}
