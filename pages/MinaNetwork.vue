...
async function fetchWallets() {
  ...
  try {
    ...
    console.error('[Mina] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateMinaWallet', {
      ...
      body: {
        userID: userID.value,
        walletName: `Mina Wallet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('Mina Protocol wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Mina] Error generating wallet:', error);
...
<template>
  <div class="cardano-network-page">
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <div class="title-section">
          ...
          <div>
            <h1>Mina Protocol</h1>
            <p class="subtitle">Manage your MINA wallets - World's lightest blockchain</p>
          </div>
        </div>
    ...
    <div class="stats-grid">
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          <span class="stat-icon">💎</span>
          <div class="stat-info">
            <span class="stat-label">Total MINA</span>
    ...
    <!-- Wallets List -->
    <n-card title="💼 Your Mina Protocol Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        <span style="font-size: 48px;">M</span>
        <h3 style="margin-top: 16px;">No Mina Protocol Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first Mina wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
        ...
        >
          <div class="wallet-header">
            <div class="wallet-title">
              <span class="wallet-emoji">M</span>
              <div>
          ...
          <div class="wallet-info">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{ wallet.balance || '0' }} MINA</span>
            </div>
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(`https://minascan.io/mainnet/account/${wallet.address}`, '_blank')">
              🔍 View on MinaScan
            </n-button>
          </div>
        </n-card>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            Mina Protocol ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
        ...
        <div style="display: flex; gap: 12px;">
          <n-button type="primary" block @click="window.open(`https://minascan.io/mainnet/account/${selectedWallet.address}`, '_blank')">
            🔍 View on MinaScan
          </n-button>
        </div>
      </div>
...
.stat-card {
  background: linear-gradient(135deg, rgba(255, 96, 59, 0.1) 0%, rgba(255, 155, 59, 0.1) 100%);
  border: 1px solid rgba(255, 96, 59, 0.2);
}
...
.wallet-card:hover {
  border-color: rgba(255, 96, 59, 0.5);
  box-shadow: 0 4px 20px rgba(255, 96, 59, 0.2);
}
