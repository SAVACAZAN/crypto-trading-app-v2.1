<template>
  <div class="energyweb-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #A0CD39, #87B32D); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              ⚡
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Energy Web Chain</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 1 / Energy</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="246">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 1 / Energy">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="EWT">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Energy Web Chain Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Energy Web Chain wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Energy Web Chain Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="⚡ About Energy Web Chain">
              <p style="line-height: 1.8; color: #666;">
                Energy Web Chain is a public, enterprise-grade blockchain platform designed specifically for the energy sector's requirements.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.energyweb.org" target="_blank">https://rpc.energyweb.org</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.energyweb.org" target="_blank">https://explorer.energyweb.org</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Energy Focused:</strong> Built for energy sector</li>
                <li>💰 <strong>Enterprise Grade:</strong> Production ready</li>
                <li>⚡ <strong>PoA Consensus:</strong> Validator-based</li>
                <li>🔐 <strong>Sustainable:</strong> Low energy consumption</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling</li>
                <li>🌍 <strong>Global Network:</strong> Energy Web ecosystem</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.energyweb.org')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.energyweb.org')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://metamask.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.gnosischain.com')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[Energy Web Chain] Load error:', error);
    message.error('Failed to load Energy Web Chain wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'EnergyWeb',
        walletName: `Energy Web Chain Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Energy Web Chain wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Energy Web Chain] Generate error:', error);
...
function openExplorer(address) {
  window.open(`https://explorer.energyweb.org/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(A0CD39, 0.05), rgba(87B32D, 0.05));
}
