...
async function fetchWallets() {
  ...
  try {
    ...
    if (response.success) {
      ...
      console.log(`[zkSync Test] Loaded ${wallets.value.length} wallets`);
    } else {
      window.$message?.error(response.message || 'Failed to fetch wallets');
    }
  } catch (error) {
    console.error('[zkSync Test] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/TestNets/generateZkSyncWallet', {
      method: 'POST',
      body: {
        userID: userID.value,
        walletName: `zkSync Testnet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('zkSync testnet wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[zkSync Test] Error generating wallet:', error);
...
<template>
  <div class="zksync-test-page">
    ...
    <div class="page-header">
      <div class="header-content">
        <div class="title-section">
          ...
          <div>
            <h1>zkSync Era Testnet</h1>
            <p class="subtitle">Layer 2 scaling solution with rich dApp ecosystem</p>
          </div>
        </div>
    ...
    <n-alert type="info" style="margin-bottom: 24px;">
      ...
      <strong>zkSync Era Network</strong><br>
      Layer 2 scaling solution with a rich dApp ecosystem. Bridge test ETH, interact with dApps, and complete testnet missions.
      <div style="margin-top: 8px; display: flex; gap: 8px;">
        <n-button size="small" type="primary" @click="window.open('https://portal.zksync.io', '_blank')">
          🚀 Portal
        </n-button>
        <n-button size="small" @click="window.open('https://goerli.bridge.zksync.io', '_blank')">
          🌉 Bridge
        </n-button>
        <n-button size="small" @click="window.open('https://zksync.io', '_blank')">
          🌐 Website
        </n-button>
      </div>
    </n-alert>
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          ...
          <div class="stat-info">
            ...
            <span class="stat-value">Layer 2</span>
          </div>
        </div>
      </n-card>
    </div>
    ...
    <n-card title="📝 Testnet Tasks" style="margin-bottom: 24px;">
      <n-checkbox-group>
        <div style="display: flex; flex-direction: column; gap: 12px;">
          <n-checkbox value="1">Bridge test ETH to zkSync</n-checkbox>
          <n-checkbox value="2">Interact with dApps</n-checkbox>
          <n-checkbox value="3">Complete testnet missions</n-checkbox>
          <n-checkbox value="4">Deploy test contracts</n-checkbox>
        </div>
      </n-checkbox-group>
    </n-card>

    <!-- Wallets List -->
    <n-card title="💼 Your zkSync Testnet Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No zkSync Testnet Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first zkSync testnet wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            zkSync Era Testnet ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
        ...
        <div style="display: flex; flex-direction: column; gap: 8px;">
          ...
          <n-button block @click="window.open('https://goerli.bridge.zksync.io', '_blank')">
            🌉 Bridge ETH
          </n-button>
