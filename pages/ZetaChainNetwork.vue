<template>
  <div class="zetachain-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #00A264, #008251); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              ⛓️
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">ZetaChain</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 1 / Omnichain</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="7000">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 1 / Omnichain">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="ZETA">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No ZetaChain Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first ZetaChain wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'ZetaChain Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="⛓️ About ZetaChain">
              <p style="line-height: 1.8; color: #666;">
                ZetaChain is a fundamental, public blockchain that enables omnichain, generic smart contracts and messaging between any blockchain.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://zetachain-evm.blockpi.network/v1/rpc/public" target="_blank">https://zetachain-evm.blockpi.network/v1/rpc/public</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.zetachain.com" target="_blank">https://explorer.zetachain.com</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Omnichain:</strong> Connect all blockchains</li>
                <li>💰 <strong>Universal Apps:</strong> Cross-chain dApps</li>
                <li>⚡ <strong>Fast Finality:</strong> Quick confirmations</li>
                <li>🔐 <strong>Decentralized:</strong> Non-custodial bridging</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling</li>
                <li>🌍 <strong>Any Chain:</strong> Bitcoin, Ethereum, Cosmos+</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.zetachain.com')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.zetachain.com')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://hub.zetachain.com')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://hub.zetachain.com')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load ZetaChain wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'ZetaChain',
        walletName: `ZetaChain Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('ZetaChain wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.zetachain.com/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(00A264, 0.05), rgba(008251, 0.05));
}
