  ...
  <div class="shiden-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #E6007A, #BB005F); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🦋
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Shiden Network</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Parachain / Kusama</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="336">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Parachain / Kusama">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="SDN">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Shiden Network Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Shiden Network wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Shiden Network Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🦋 About Shiden Network">
              <p style="line-height: 1.8; color: #666;">
                Shiden is a multi-chain dApp hub on Kusama, serving as Astar's canary network with real economic value and cutting-edge features.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://evm.shiden.astar.network" target="_blank">https://evm.shiden.astar.network</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://shiden.blockscout.com" target="_blank">https://shiden.blockscout.com</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Kusama Parachain:</strong> Canary network</li>
                <li>💰 <strong>dApp Staking:</strong> Developer rewards</li>
                <li>⚡ <strong>Multi-VM:</strong> EVM + WASM support</li>
                <li>🔐 <strong>Battle Testing:</strong> Innovation first</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling</li>
                <li>🌍 <strong>Cross-Chain:</strong> XCM ready</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://shiden.astar.network')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://shiden.blockscout.com')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://portal.astar.network')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://portal.astar.network/bridge')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[Shiden Network] Load error:', error);
    message.error('Failed to load Shiden Network wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Shiden',
        walletName: `Shiden Network Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Shiden Network wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Shiden Network] Generate error:', error);
...
function openExplorer(address) {
  window.open(`https://shiden.blockscout.com/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(E6007A, 0.05), rgba(BB005F, 0.05));
}
