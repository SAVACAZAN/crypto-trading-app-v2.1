  ...
  <div class="sei-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #BB1E1E, #941818); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🌊
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Sei Network</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 1 / DeFi</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="1329">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 1 / DeFi">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="SEI">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Sei Network Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Sei Network wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Sei Network Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🌊 About Sei Network">
              <p style="line-height: 1.8; color: #666;">
                Sei is the fastest Layer 1 blockchain, optimized for trading. It features parallelized EVM and provides the best infrastructure for DeFi applications.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://evm-rpc.sei-apis.com" target="_blank">https://evm-rpc.sei-apis.com</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://seitrace.com" target="_blank">https://seitrace.com</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Fastest L1:</strong> 390ms finality</li>
                <li>💰 <strong>Trading Optimized:</strong> Built-in order matching</li>
                <li>⚡ <strong>20,000 TPS:</strong> High throughput</li>
                <li>🔐 <strong>Twin Turbo:</strong> Parallelized EVM</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum + CosmWasm</li>
                <li>🌍 <strong>DeFi Focus:</strong> Optimized for trading</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.sei.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://seitrace.com')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://app.sei.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.sei.io')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[Sei Network] Load error:', error);
    message.error('Failed to load Sei Network wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Sei',
        walletName: `Sei Network Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Sei Network wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Sei Network] Generate error:', error);
...
function openExplorer(address) {
  window.open(`https://seitrace.com/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(BB1E1E, 0.05), rgba(941818, 0.05));
}
