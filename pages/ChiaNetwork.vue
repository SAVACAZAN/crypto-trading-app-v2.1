...
async function fetchWallets() {
  ...
  try {
    ...
    console.error('[Chia] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateChiaWallet', {
      ...
      body: {
        userID: userID.value,
        walletName: `Chia Wallet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('Chia wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Chia] Error generating wallet:', error);
...
<template>
  <div class="cardano-network-page">
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <div class="title-section">
          ...
          <div>
            <h1>Chia Network</h1>
            <p class="subtitle">Manage your XCH wallets - Green Blockchain</p>
          </div>
        </div>
    ...
    <div class="stats-grid">
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          <span class="stat-icon">💎</span>
          <div class="stat-info">
            <span class="stat-label">Total XCH</span>
    ...
    <!-- Wallets List -->
    <n-card title="💼 Your Chia Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No Chia Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first Chia wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
        ...
        >
          ...
          <div class="wallet-info">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{ wallet.balance || '0' }} XCH</span>
            </div>
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(`https://www.chiaexplorer.com/blockchain/address/${wallet.address}`, '_blank')">
              🔍 View on Chia Explorer
            </n-button>
          </div>
        </n-card>
      </div>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            Chia ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
        ...
        <div style="display: flex; gap: 12px;">
          <n-button type="primary" block @click="window.open(`https://www.chiaexplorer.com/blockchain/address/${selectedWallet.address}`, '_blank')">
            🔍 View on Chia Explorer
          </n-button>
        </div>
      </div>
...
.stat-card {
  background: linear-gradient(135deg, rgba(58, 172, 89, 0.1) 0%, rgba(100, 200, 130, 0.1) 100%);
  border: 1px solid rgba(58, 172, 89, 0.3);
}
...
.wallet-card:hover {
  border-color: rgba(58, 172, 89, 0.5);
  box-shadow: 0 4px 20px rgba(58, 172, 89, 0.2);
}
