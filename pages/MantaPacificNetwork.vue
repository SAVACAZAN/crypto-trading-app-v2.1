<template>
  <div class="mantapacific-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #6B46C1, #553C9A); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🦈
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Manta Pacific</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="169">
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Manta Pacific Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Manta Pacific wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Manta Pacific Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🦈 About Manta Pacific">
              <p style="line-height: 1.8; color: #666;">
                Manta Pacific is a modular Layer 2 ecosystem on Ethereum, providing scalable and low-cost solutions for Web3 applications with native privacy features.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://pacific-rpc.manta.network/http" target="_blank">https://pacific-rpc.manta.network/http</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://pacific-explorer.manta.network" target="_blank">https://pacific-explorer.manta.network</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Modular L2:</strong> Flexible architecture</li>
                <li>💰 <strong>Low Fees:</strong> Cost-effective transactions</li>
                <li>⚡ <strong>High Speed:</strong> Fast finality</li>
                <li>🔐 <strong>Privacy Features:</strong> Optional privacy layer</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling</li>
                <li>🌍 <strong>Web3 Focus:</strong> Built for dApps</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://pacific.manta.network')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://pacific-explorer.manta.network')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://pacific-bridge.manta.network')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://pacific-bridge.manta.network')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Manta Pacific wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        ...
        walletName: `Manta Pacific Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Manta Pacific wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://pacific-explorer.manta.network/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(6B46C1, 0.05), rgba(553C9A, 0.05));
}
