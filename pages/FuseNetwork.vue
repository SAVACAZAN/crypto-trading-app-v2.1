  ...
  <div class="fuse-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #B0F566, #95D951); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              💳
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Fuse Network</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 1 / Payments</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="122">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 1 / Payments">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="FUSE">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Fuse Network Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Fuse Network wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Fuse Network Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="💳 About Fuse Network">
              <p style="line-height: 1.8; color: #666;">
                Fuse is a fast, mobile-first, and EVM-compatible blockchain designed to enable businesses to create and operate wallets and payments infrastructure.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.fuse.io" target="_blank">https://rpc.fuse.io</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.fuse.io" target="_blank">https://explorer.fuse.io</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Mobile First:</strong> Built for mobile apps</li>
                <li>💰 <strong>Low Fees:</strong> Affordable transactions</li>
                <li>⚡ <strong>5-Second Blocks:</strong> Fast payments</li>
                <li>🔐 <strong>DPoS:</strong> Delegated Proof of Stake</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum support</li>
                <li>🌍 <strong>Payments Focus:</strong> Business-ready</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://fuse.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.fuse.io')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.fuse.io')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.fuse.io')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[Fuse Network] Load error:', error);
    message.error('Failed to load Fuse Network wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Fuse',
        walletName: `Fuse Network Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Fuse Network wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Fuse Network] Generate error:', error);
...
.header-card {
  background: linear-gradient(135deg, rgba(B0F566, 0.05), rgba(95D951, 0.05));
}
