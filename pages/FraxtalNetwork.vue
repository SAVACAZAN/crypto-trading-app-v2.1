<template>
  <div class="fraxtal-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #000000, #1A1A1A); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              ❄️
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Fraxtal</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="252">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="frxETH">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Fraxtal Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Fraxtal wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Fraxtal Wallet ' + (index + 1)}`" hoverable>
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="Balance">
                  <div>
                    <div>{{ wallet.balance || '0' }} frxETH</div>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="❄️ About Fraxtal">
              <p style="line-height: 1.8; color: #666;">
                Fraxtal is a modular Layer 2 blockchain built by Frax Finance using the OP Stack. It features innovative tokenomics with incentivized block space.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.frax.com" target="_blank">https://rpc.frax.com</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://fraxscan.com" target="_blank">https://fraxscan.com</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>OP Stack:</strong> Optimism-based L2</li>
                <li>💰 <strong>Frax Ecosystem:</strong> Integrated with Frax</li>
                <li>⚡ <strong>Fast & Cheap:</strong> Efficient transactions</li>
                <li>🔐 <strong>Modular Design:</strong> Flexible architecture</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Solidity support</li>
                <li>🌍 <strong>DeFi Focus:</strong> Built for finance</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://frax.finance')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://fraxscan.com')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://app.frax.finance')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.frax.finance')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Fraxtal wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Fraxtal',
        walletName: `Fraxtal Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Fraxtal wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://fraxscan.com/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(000000, 0.05), rgba(1A1A1A, 0.05));
}
