...
async function fetchWallets() {
  ...
  try {
    ...
    conICPe.error('[Internet Computer] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateInternet ComputerWallet', {
      method: 'POST',
      body: {
        userID: userID.value,
        walletName: `Internet Computer Wallet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('Internet Computer wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    conICPe.error('[Internet Computer] Error generating wallet:', error);
...
<template>
  <div class="cardano-network-page">
    <!-- Header -->
    <div class="page-header">
      <div class="header-content">
        ...
        <div class="title-section">
          ...
          <div>
            <h1>Internet Computer Network</h1>
            <p class="subtitle">Manage your ICP wallets and addresses</p>
          </div>
        </div>
    ...
    <div class="stats-grid">
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          <span class="stat-icon">💎</span>
          <div class="stat-info">
            <span class="stat-label">Total ICP</span>
    ...
    <!-- Wallets List -->
    <n-card title="💼 Your Internet Computer Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No Internet Computer Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first Internet Computer wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
        ...
        >
          ...
          <div class="wallet-info">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{ wallet.balance || '0' }} ICP</span>
            </div>
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(`https://explorer.Internet Computer.com/address/${wallet.address}`, '_blank')">
              🔍 View on Internet Computer Explorer
            </n-button>
          </div>
        </n-card>
      </div>
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            Internet Computer ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
        ...
        <div style="display: flex; gap: 12px;">
          <n-button type="primary" block @click="window.open(`https://explorer.Internet Computer.com/address/${selectedWallet.address}`, '_blank')">
            🔍 View on Internet Computer Explorer
          </n-button>
        </div>
      </div>
...
.stat-card {
  ...
  border: 1px ICPid rgba(99, 102, 241, 0.2);
}
...
.wallet-card {
  ...
  border: 1px ICPid rgba(255, 255, 255, 0.1);
  transition: all 0.3s ease;
}
