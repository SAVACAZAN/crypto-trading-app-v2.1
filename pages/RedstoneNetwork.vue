<template>
  <div class="redstone-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #FF0000, #CC0000); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🔴
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Redstone</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="690">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="RED">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Redstone Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Redstone wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Redstone Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🔴 About Redstone">
              <p style="line-height: 1.8; color: #666;">
                Redstone is an Ethereum Layer 2 network using Optimistic Rollup technology, designed for high-performance decentralized applications with low transaction costs.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.redstonechain.com" target="_blank">https://rpc.redstonechain.com</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.redstone.xyz" target="_blank">https://explorer.redstone.xyz</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Optimistic Rollup:</strong> Ethereum L2 scaling</li>
                <li>💰 <strong>Low Fees:</strong> Cost-effective transactions</li>
                <li>⚡ <strong>Fast Performance:</strong> Quick block times</li>
                <li>🔐 <strong>Ethereum Security:</strong> Secured by Ethereum</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Full Solidity support</li>
                <li>🌍 <strong>DeFi Ready:</strong> Optimized for DeFi applications</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://redstone.xyz')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.redstone.xyz')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://bridge.redstone.xyz')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.redstone.xyz')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Redstone wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Redstone',
        walletName: `Redstone Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Redstone wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.redstone.xyz/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(FF0000, 0.05), rgba(CC0000, 0.05));
}
