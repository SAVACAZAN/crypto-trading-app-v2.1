...
async function fetchWallets() {
  ...
  try {
    ...
    console.error('[IOTA] Error fetching wallets:', error);
...
async function handleGenerateWallet() {
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateIOTAWallet', {
      ...
      body: {
        userID: userID.value,
        walletName: `IOTA Wallet ${Date.now()}`
      }
    });

    if (response.success) {
      window.$message?.success('IOTA wallet generated successfully!');
      await fetchWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[IOTA] Error generating wallet:', error);
...
<template>
  <div class="cardano-network-page">
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <div class="title-section">
          ...
          <div>
            <h1>IOTA Network</h1>
            <p class="subtitle">Manage your MIOTA wallets - The Tangle (DAG)</p>
          </div>
        </div>
    ...
    <div class="stats-grid">
      ...
      <n-card class="stat-card">
        <div class="stat-content">
          <span class="stat-icon">💎</span>
          <div class="stat-info">
            <span class="stat-label">Total MIOTA</span>
    ...
    <!-- Wallets List -->
    <n-card title="💼 Your IOTA Wallets" style="margin-top: 24px;">
      ...
      <div v-else-if="wallets.length === 0" style="text-align: center; padding: 40px;">
        ...
        <h3 style="margin-top: 16px;">No IOTA Wallets Yet</h3>
        <p style="color: rgba(255,255,255,0.6); margin-bottom: 24px;">
          Generate your first IOTA wallet to get started
        </p>
        <n-button type="primary" @click="handleGenerateWallet" :loading="generatingWallet">
          ➕ Generate Wallet
        </n-button>
        ...
        >
          ...
          <div class="wallet-info">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{ wallet.balance || '0' }} MIOTA</span>
            </div>
          ...
          <div class="wallet-actions">
            <n-button size="small" type="primary" ghost @click="window.open(`https://explorer.iota.org/mainnet/addr/${wallet.address}`, '_blank')">
      ...
      <div v-if="selectedWallet" class="wallet-details">
        <n-descriptions :column="1" bordered size="small">
          ...
          <n-descriptions-item label="Network">
            IOTA ({{ selectedWallet.networkSymbol }})
          </n-descriptions-item>
          ...
          <n-descriptions-item label="Balance">
            {{ selectedWallet.balance || '0' }} MIOTA
          </n-descriptions-item>
        ...
        <div style="display: flex; gap: 12px;">
          <n-button type="primary" block @click="window.open(`https://explorer.iota.org/mainnet/addr/${selectedWallet.address}`, '_blank')">
            🔍 View on IOTA Explorer
          </n-button>
        </div>
      </div>
...
.stat-card {
  background: linear-gradient(135deg, rgba(19, 31, 55, 0.3) 0%, rgba(60, 80, 120, 0.3) 100%);
  border: 1px solid rgba(19, 31, 55, 0.5);
}
...
.wallet-card:hover {
  border-color: rgba(19, 31, 55, 0.8);
  box-shadow: 0 4px 20px rgba(19, 31, 55, 0.3);
}
