<template>
  <div class="viction-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #6C5CE7, #5849C4); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🎯
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Viction</h2>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="88">
        ...
        <n-gi>
          <n-statistic label="Native Token" value="VIC">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Viction Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Viction wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Viction Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🎯 About Viction">
              <p style="line-height: 1.8; color: #666;">
                Viction (formerly TomoChain) is a people-centric Layer 1 blockchain offering zero-gas transactions and a focus on user experience.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://rpc.viction.xyz" target="_blank">https://rpc.viction.xyz</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://www.vicscan.xyz" target="_blank">https://www.vicscan.xyz</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>PoSV Consensus:</strong> Proof of Stake Voting</li>
                <li>💰 <strong>Zero Gas:</strong> Gasless transactions</li>
                <li>⚡ <strong>2-Second Blocks:</strong> Fast finality</li>
                <li>🔐 <strong>150 Masternodes:</strong> Decentralized</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling</li>
                <li>🌍 <strong>DeFi Ready:</strong> Multiple dApps</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.viction.xyz')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://www.vicscan.xyz')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://wallet.viction.xyz')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.viction.xyz')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Viction wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Viction',
        walletName: `Viction Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Viction wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://www.vicscan.xyz/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(6C5CE7, 0.05), rgba(5849C4, 0.05));
}
