  ...
  <div class="bnb-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #F3BA2F, #FFD700); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🟡
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">BNB Chain</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Binance Smart Chain</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="56">
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No BNB Chain Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first BNB Chain wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'BNB Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🟡 About BNB Chain">
              <p style="line-height: 1.8; color: #666;">
                BNB Chain (formerly Binance Smart Chain) is a blockchain network built for running smart contract-based applications.
                It operates in parallel with Binance Chain, offering fast, low-cost transactions and full EVM compatibility.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://bsc-dataseed.binance.org" target="_blank">https://bsc-dataseed.binance.org</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://bscscan.com" target="_blank">https://bscscan.com</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>High Performance:</strong> 3-second block time, ~300 TPS</li>
                <li>💰 <strong>Low Fees:</strong> Transaction costs as low as $0.10</li>
                <li>⚡ <strong>Fast Finality:</strong> Transactions confirmed in seconds</li>
                <li>🔐 <strong>Dual Chain:</strong> Works alongside Binance Chain</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Supports Ethereum tools and dApps</li>
                <li>🌍 <strong>Large Ecosystem:</strong> 1000+ projects and DeFi protocols</li>
                <li>💱 <strong>CEX Integration:</strong> Seamless Binance exchange integration</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.bnbchain.org')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://bscscan.com')" block>
                  🔍 BscScan Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://www.binance.com/en/wallet')" block>
                  👛 Binance Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://www.binance.org/en/bridge')" block>
                  🌉 Binance Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[BNB Chain] Load error:', error);
    message.error('Failed to load BNB Chain wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'BNB',
        walletName: `BNB Chain Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('BNB Chain wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[BNB Chain] Generate error:', error);
...
.header-card {
  background: linear-gradient(135deg, rgba(243, 186, 47, 0.05), rgba(255, 215, 0, 0.05));
}
