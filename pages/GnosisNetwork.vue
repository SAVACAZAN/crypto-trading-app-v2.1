  ...
  <div class="gnosis-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <div>
              <h2 style="margin: 0; font-size: 24px;">Gnosis Chain</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Sidechain - Community-owned network</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="100">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Sidechain">
            <template #prefix>🌐</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="xDAI">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Gnosis Chain Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Gnosis Chain wallet to get started</p>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <div style="padding: 20px;">
            ...
            <n-descriptions bordered :column="2">
              <n-descriptions-item label="Network Name">Gnosis Chain</n-descriptions-item>
              <n-descriptions-item label="Chain ID">100</n-descriptions-item>
              <n-descriptions-item label="Native Currency">xDAI</n-descriptions-item>
              <n-descriptions-item label="Network Type">Sidechain</n-descriptions-item>
            </n-descriptions>
...
async function loadWallets() {
  ...
  try {
    ...
    console.error('[Gnosis Chain] Load error:', error);
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Gnosis',
        walletName: `Gnosis Chain Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      window.$message?.success('Gnosis Chain wallet generated successfully!');
      await loadWallets();
    } else {
      throw new Error(response.message || 'Failed to generate wallet');
    }
  } catch (error) {
    console.error('[Gnosis Chain] Generate error:', error);
