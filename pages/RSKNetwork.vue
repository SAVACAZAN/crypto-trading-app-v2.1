  ...
  <div class="rsk-network-page">
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #FF9900, #CC7A00); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🟠
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">RSK</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Sidechain / Bitcoin</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="30">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Sidechain / Bitcoin">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="RBTC">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No RSK Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first RSK wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'RSK Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🟠 About RSK">
              <p style="line-height: 1.8; color: #666;">
                RSK (Rootstock) is a Bitcoin sidechain that brings smart contract functionality to Bitcoin, secured by Bitcoin's proof-of-work.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://public-node.rsk.co" target="_blank">https://public-node.rsk.co</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://explorer.rsk.co" target="_blank">https://explorer.rsk.co</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Bitcoin Sidechain:</strong> BTC-secured</li>
                <li>💰 <strong>Bitcoin Bridge:</strong> 2-way peg</li>
                <li>⚡ <strong>30-Second Blocks:</strong> Fast for Bitcoin</li>
                <li>🔐 <strong>Merge Mining:</strong> Bitcoin PoW</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum tooling</li>
                <li>🌍 <strong>DeFi on Bitcoin:</strong> Smart contracts</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://rootstock.io')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://explorer.rsk.co')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://www.defiant.app')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://tokenbridge.rsk.co')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load RSK wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'RSK',
        walletName: `RSK Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('RSK wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://explorer.rsk.co/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(FF9900, 0.05), rgba(CC7A00, 0.05));
}
