<template>
  <div class="wanchain-network-page">
    <!-- Back Button -->
    ...
    <n-card class="header-card" style="margin-bottom: 24px;">
      <template #header>
        <div style="display: flex; align-items: center; gap: 16px; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 48px; height: 48px; background: linear-gradient(135deg, #136AAD, #0F5489); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 28px;">
              🌐
            </div>
            <div>
              <h2 style="margin: 0; font-size: 24px;">Wanchain</h2>
              <p style="margin: 0; color: #666; font-size: 14px;">Layer 1 / Cross-Chain</p>
            </div>
          </div>
      ...
      <n-grid cols="2 s:3 m:4" responsive="screen" :x-gap="16" :y-gap="16">
        <n-gi>
          <n-statistic label="Chain ID" value="888">
            <template #prefix>#</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Network Type" value="Layer 1 / Cross-Chain">
            <template #prefix>⚡</template>
          </n-statistic>
        </n-gi>
        <n-gi>
          <n-statistic label="Native Token" value="WAN">
            <template #prefix>💎</template>
          </n-statistic>
        </n-gi>
        <n-gi>
      ...
      <n-tabs type="line" animated>
        ...
        <n-tab-pane name="wallets" tab="👛 My Wallets">
          ...
          <div v-else-if="wallets.length === 0" style="text-align: center; padding: 60px 20px;">
            ...
            <h3 style="margin: 0 0 8px 0;">No Wanchain Wallets Yet</h3>
            <p style="color: #666; margin-bottom: 24px;">Generate your first Wanchain wallet to get started</p>
          ...
          <n-space v-else vertical :size="16">
            <n-card v-for="(wallet, index) in wallets" :key="wallet._id" :title="`${wallet.walletName || 'Wanchain Wallet ' + (index + 1)}`" hoverable>
        ...
        <n-tab-pane name="info" tab="ℹ️ Network Info">
          <n-space vertical :size="16">
            <n-card title="🌐 About Wanchain">
              <p style="line-height: 1.8; color: #666;">
                Wanchain is a decentralized blockchain interoperability solution connecting isolated blockchain networks. It enables cross-chain transfers and smart contracts.
              </p>
            </n-card>
            ...
            <n-card title="🔗 Network Details">
              <n-descriptions :column="1" bordered>
                ...
                <n-descriptions-item label="RPC URL">
                  <a href="https://gwan-ssl.wandevs.org:56891" target="_blank">https://gwan-ssl.wandevs.org:56891</a>
                </n-descriptions-item>
                <n-descriptions-item label="Block Explorer">
                  <a href="https://wanscan.org" target="_blank">https://wanscan.org</a>
                </n-descriptions-item>
              </n-descriptions>
            </n-card>
            ...
            <n-card title="✨ Key Features">
              <ul style="line-height: 2; color: #666;">
                <li>🚀 <strong>Cross-Chain:</strong> Connect blockchains</li>
                <li>💰 <strong>Low Fees:</strong> Efficient operations</li>
                <li>⚡ <strong>Fast Bridges:</strong> Quick cross-chain</li>
                <li>🔐 <strong>Secure:</strong> SMPC technology</li>
                <li>🛠️ <strong>EVM Compatible:</strong> Ethereum support</li>
                <li>🌍 <strong>Multi-Chain:</strong> 15+ blockchains</li>
              </ul>
            </n-card>

            <n-card title="🔗 Useful Links">
              <n-space vertical :size="12">
                <n-button type="primary" @click="openUrl('https://www.wanchain.org')" block>
                  🌐 Official Website
                </n-button>
                <n-button type="info" @click="openUrl('https://wanscan.org')" block>
                  🔍 Block Explorer
                </n-button>
                <n-button type="success" @click="openUrl('https://www.wanchain.org/getstarted')" block>
                  👛 Wallet
                </n-button>
                <n-button type="warning" @click="openUrl('https://bridge.wanchain.org')" block>
                  🌉 Bridge
                </n-button>
              </n-space>
            </n-card>
...
async function loadWallets() {
  ...
  try {
    ...
    message.error('Failed to load Wanchain wallets');
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    const response = await $fetch('/api/v1/Wallets/generateEVMWallet', {
      ...
      body: {
        userID: userID.value,
        network: 'Wanchain',
        walletName: `Wanchain Wallet ${wallets.value.length + 1}`
      }
    });

    if (response.success) {
      message.success('Wanchain wallet generated successfully!');
      await loadWallets();
    } else {
...
function openExplorer(address) {
  window.open(`https://wanscan.org/address/${address}`, '_blank');
}
...
.header-card {
  background: linear-gradient(135deg, rgba(136AAD, 0.05), rgba(0F5489, 0.05));
}
